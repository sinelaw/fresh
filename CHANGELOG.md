...
* **Review Diff opens large diffs almost instantly** - rendering git's own diff output directly instead of building every row by hand cuts opening a 524-file range review from ~16s to ~2s, and folding/unfolding it all from ~17s/4s to ~4s/0.2s (#3056).
...
* **Multi-cursor editing works reliably again** - a keystroke that only moves the cursor no longer vanishes with multiple cursors live, and a plain mouse click now collapses back to a single cursor instead of leaving strays that edit off-screen (#3125, reported by @X-Ryl669).
...
* **The Welcome screen's caret and its focus stay on the same thing**: Tab brings the caret to the control it focuses, moving the caret onto a paragraph clears focus so `Enter` does nothing, and landing on a control — the file finder's field included — focuses it. Its level banners also breathe: a blank row now separates each rule from the sentence under it (#3194).
...
* **Review Diff's files sidebar shows whole filenames** when it opens, instead of eliding them to a few characters until the panel divider was dragged. The geometry a pane is born with was never reported to plugins, and for a side panel that is the only announcement there is — so every plugin-drawn panel laid its rows out to a guess (#3188).
...
* **A hover highlight no longer lingers while you type.** A pointer left resting on a list row kept its band — beside the review sidebar's real selection, that read as two "you are here" markers. Pointer feedback now ends at the first keystroke and returns when the pointer moves (#3188).
* **Review Diff, Git Log and Side-by-Side Diff parse `git`'s output correctly under more git configs** - a quoted non-ASCII path, a suppressed blank context line, `diff.relative`, a changed submodule, or a signed commit no longer break file matching, line numbers, or `Enter`-to-open (#3199).
* **`fresh --cmd update` no longer fails with a bare 403 when GitHub's shared API rate limit runs out** - it now resolves the latest release the same way `install.sh` does (no API call needed to check for updates), honours `GITHUB_TOKEN`/`GH_TOKEN` to raise the limit, and offers `--skip-attestation` to install on the checksum alone when only the attestation check is rate-limited (#3198).
