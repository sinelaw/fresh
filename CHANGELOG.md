...
* **The status bar's line number follows an added cursor** - after `Ctrl+Alt+Down` the bar read `Ln 1` next to the new cursor's column, and stayed there through further adds until the first arrow key made it jump straight to `Ln 3`, so one `Right` looked like it had moved two lines. The line number is a cache that only real cursor moves refreshed, while the column is read live; adding or removing a cursor changes which cursor is the primary without moving it, and a multi-cursor edit writes cursor positions without a move at all. All three now refresh it (#3167).
