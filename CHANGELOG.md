...
* **LSP diagnostics no longer get stuck reporting errors you already fixed** - the editor talks to a language server over a bounded queue, and a server that stopped draining it (a stalled `gopls`, a long reindex) eventually backed that queue up until a `didChange` could not be enqueued. The notification carries a *range*, so quietly losing one left the server holding a document that no longer matched the buffer for the rest of its life: it kept reporting syntax errors against text nobody had written, and further editing only compounded the divergence rather than repairing it. A change that cannot be queued is now recorded rather than dropped - the editor re-sends the whole document as soon as the server has room again, and treats anything the server derived from its stale copy in the meantime as out of date (#3038, reported by @thedadams).
