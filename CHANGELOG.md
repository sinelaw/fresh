...
* **Piped-in text no longer leaves a copy in `/tmp` for a day** - `… | fresh -` spools stdin to `/tmp/fresh-stdin-<pid>.tmp` so a large input can be paged in lazily, and nothing deleted it on the way out; the file waited for the 24-hour sweep of stale logs, which is a lot of disk to leave behind after piping in a decompressed log. The file is now removed when fresh exits, including when it exits by crashing (#3134, reported by @Korkman).
