...
* **File Explorer**: a tree with more entries than fit now draws a scrollbar, and it can be dragged to scroll the tree. The bar hides itself when everything fits, and it no longer sits on the row's status marker - hovering the bar used to pop the tooltip of the file behind it while hovering the marker itself did nothing (#2859, reported by @asukaminato0721).
