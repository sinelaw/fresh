...
* **Shaded pane edges** - the two rows at the top and bottom of each pane are painted part of the way up from their background, so text trails off at the edge instead of being cut mid-line, and a scroll has somewhere to come from and go to. The row hard against the edge is dimmest, the one inside it a step brighter, and the third row in is normal. It is constant rather than animated: the same rows are shaded whether the view is moving or still, so nothing flickers and scrolling is a plain shift of the text through a fixed gradient. Text sitting on a selection or the current-line highlight fades into that band rather than into the editor background. The top edge only shades when there is something above it, so the opening lines of a file are not dimmed for no reason. Off with `editor.viewport_edge_fade`.
* **Smooth wheel scrolling** - a wheel notch now walks the view a line at a time instead of jumping it three lines at once. The first line lands immediately, so the view still answers the wheel on the same frame, and the rest follow at about a line a frame. The walk is paced by the clock rather than by frames, so a terminal too slow to show it collapses the remainder back into a single jump rather than lagging behind the wheel. Off with `editor.smooth_scroll`; how far a notch takes the view is `editor.mouse_wheel_scroll_lines` (default 3, unchanged).
...
* **Markdown types brackets and quotes literally** - prose types a backtick, a quote or a bracket as literal text far more often than as the opening half of a pair, so a `` ` `` before a word turned into `` `` `` around the caret and every `"` came out doubled. Markdown files now ship with both auto-close and auto-surround off (`languages.markdown.auto_close` and `.auto_surround`, set either to `true` to opt back in); every other language is unchanged. The two settings are now genuinely independent, as they were always documented to be - turning auto-close off for a language used to silently take auto-surround down with it.
...
* **A `deno.json` no longer takes TypeScript away from a project without Deno** - a `deno.json` or `deno.jsonc` in the project root switches JavaScript and TypeScript to `deno lsp`, which is right for a Deno project and wrong for the npm/pnpm workspace that merely contains one: with Deno not installed, both languages were left with a server that could not start while a working `typescript-language-server` sat unused on `PATH`. The switch now happens only when the Deno runtime is there, and it moved out of the editor into a plugin (`deno_lsp.ts`) you can disable or copy (#2981, reported by @atk, who diagnosed it and proposed the runtime check).
* **Deleting a keymap binding in the keybinding editor now deletes it** - `d` on a built-in row used to write a `noop` override for the same key: the row came back as `noop`, and the key was left dead rather than free, because a `noop` is itself a binding and buries whatever a `global` entry or the parent keymap bound underneath it. Delete now writes an `unbind` entry (`{"key": "g", "modifiers": ["alt"], "action": "unbind", "when": "normal"}`), which takes the built-in binding out of scope: the row is gone, and the key falls through to whatever else binds it. Removing a binding also takes its terminal aliases along (`Ctrl+/` and its `Ctrl+7` spelling), and a removed plugin mode binding stays removed when the plugin registers it again. The `noop` override is still there for when you want a key dead rather than free - `x` on any row disables it that way.
* **A single-key binding on a keymap chord's prefix now fires** - on the emacs keymap, binding `Alt+G` to `goto_line` did nothing: the keymap's `Alt+G G` / `Alt+G Alt+G` chords held `Alt+G` as a pending prefix, and the keybinding editor reported no conflict. A user's single-key binding now outranks a built-in chord's prefix, the same way every user binding outranks every built-in one, and a chord that is removed or disabled releases its prefix once no live chord starts with it (#3171).
* **LSP requests that time out say so** - a language server can finish `initialize`, so the editor reports it ready, and then answer nothing at all. Requests are cancelled after 30 seconds and the feature that asked gets an empty answer, which is how a server that never replied came to be reported as `No definition found` and `No hover information available` while the status bar read `LSP (python) ready` throughout. An expired request is now named on the status bar, an empty result that follows one explains itself instead of posing as an answer, and a server that leaves two requests in a row unanswered shows as `LSP (stuck)` with `(not responding)` in the LSP-servers popup until it answers something. The editor also now pushes a `workspace/didChangeConfiguration` right after `initialized`, as VS Code, Neovim and Helix do - some servers only finish setting up on receiving it, and pyright 1.1.408 was one: it never answered a client that did not send it (#2197).
* **An inlay hint no longer jumps a line while you type at it** - hints are anchored to a byte position in the buffer, and a hint that renders *before* its anchor was being carried along by text inserted at that exact position: pressing Enter with the cursor on the anchor of a `let d: Duration` hint moved the hint onto the next line, against the tail of the split, until the next hints arrived from the server about a second later. Insertions at a hint's anchor now stay behind it, so the hint keeps annotating what it was attached to and the screen is right on the frame after the edit rather than a refresh later (#722).
* **`curl ... | install.sh` no longer fails behind a shared IP** - the installer called the GitHub API to find out what to download, and that API allows 60 requests per hour per source IP, so from a NAT, a CI runner, or an office network it answered `403` and the install died reporting `curl: (3) bad range in URL position 3` - which named curl's URL parser rather than the rate limit that actually stopped it. The API is gone: assets are fetched from `/releases/latest/download`, which needs no account, no token, and no credential of any kind, and is not rate limited that way. Anonymous internet access to `github.com` is now the whole requirement. A failed lookup can also no longer be mistaken for a successful one - diagnostics go to stderr instead of stdout, so an error message can never be captured and used in place of the URL it was reporting the absence of - and a refused download is now reported as a refused download rather than as a missing release.
* **The published install one-liner now fails visibly** - it used `curl -sL`, which hides curl's own errors, so when the fetch itself failed GitHub's `404: Not Found` body was piped into `sh` and *executed*, and the user saw `sh: 1: 404:: not found` - naming neither the URL nor the real problem. The documented command is now `curl -fsSL`, which sends nothing to the shell on an HTTP error and prints what actually went wrong.
* **Typing over an auto-closed `)` lands past it even when another cursor inserts one** - with `xx` above `a()`, one cursor at the end of `xx` and one between the parens of `a()`, typing `)` gave the first line its `)` but left the second cursor *inside* its parens, so the `;` typed next produced `a(;)` - the same symptom #3125 had from the other end, in the one arrangement that fix did not cover. A cursor stepping over an existing delimiter emits only a move, in the coordinates of the buffer before the keystroke, while the multi-cursor edit path takes a move from a cursor that inserted nothing as already accounting for the other cursors' insertions. The skip-over moves are now rebased onto the other cursors' edits before they are applied, so the stepping cursor lands one byte later when an earlier cursor inserted one (#3166).
* **Keystrokes that only move the cursor no longer vanish when multiple cursors are live** - typing the `)` that auto-close had already put there did nothing at all with more than one cursor: every cursor stayed *inside* the parens, so the `;` typed next landed there too and `a.open();` came out as `a.open(;)` on every line. Typing a closing delimiter that is already in front of the cursor is a skip-over - it steps the cursor past the character rather than editing anything - and the multi-cursor path applied keystrokes through a bulk edit that refused any event list containing no edits, so a keystroke whose whole effect was to move cursors was discarded for every cursor at once. The bulk path now applies such a list as one batch, which also restores backspace in virtual space with several cursors, where there is nothing to delete yet and the keypress likewise only moves (#3125, reported by @X-Ryl669).
...
* **Dragging a divider keeps up with the pointer** - pulling the border between two split panes, the file explorer's right edge, or the orchestrator dock's was sluggish and lagged a second or more behind the mouse. A terminal reports a drag as one event per cell the pointer crosses, and only the button-up kind of motion was being collapsed to its latest report - so every intermediate cell of a drag cost a full re-layout and a full repaint, and a repaint costs more than the 60fps frame budget, leaving the editor a frame behind for every cell. Drag motion now collapses the same way free motion always has, and a drag report that doesn't actually move the divider (sliding *along* it, or a width that rounds back to the column it was already on) no longer reflows at all.
* **Review Diff: `D` on a fully staged file discards it for real** - discarding a file whose change was entirely staged reported `Discarded: <file>` and changed nothing at all: the discard ran `git checkout -- <file>`, which rewrites the working tree *from the index*, and for a fully staged change the two already agree. A file-level discard now takes the file back to HEAD - index and working tree both, covering a staged add (which `git checkout HEAD -- <file>` cannot touch at all), a staged delete, and a rename's two paths - and the outcome is checked against `git status` before anything is reported, so a discard that could not happen now says so, with git's reason, instead of announcing an irreversible success it never performed (#2318).
* **Review Diff follows the working tree** - an open panel now picks up changes made outside the editor - a `git` command, an agent editing files in another terminal - instead of showing whatever git said at the moment it opened. It re-reads git on a timer and rebuilds only when the answer actually differs, so a quiet repo costs nothing on screen: cursor, folds and the status line stay where they were. `W` still toggles the watch, which now also covers external changes and is on by default. And re-running **Review Diff** while a review is open refreshes that panel instead of opening a second one - the old workaround for the missing refresh piled up `*Review Diff* 1`, `*Review Diff* 2`, … tabs, one per attempt (#3126, reported by @asukaminato0721).
...
* **The space below the last line follows the theme background** - rows past the end of the buffer were painted with a shade derived from `editor.bg` rather than with a colour the theme actually names, so a short file sat above a grey strip that belonged to no theme. Those rows now use the theme's editor background - and follow it onto the terminal's own background when `use_terminal_bg` is on, where the band used to stay opaque; the `~` tildes still mark where the buffer ends. A theme that wants the post-EOF area shaded says so explicitly with `editor.after_eof_bg` (#779). One exception: a theme you saved through the graphical theme editor before this release has the old derived shade written into its JSON, so it keeps it until you clear that field.
* **LSP diagnostics no longer get stuck reporting errors you already fixed** - the editor talks to a language server over a bounded queue, and a server that stopped draining it (a stalled `gopls`, a long reindex) eventually backed that queue up until a `didChange` could not be enqueued. The notification carries a *range*, so quietly losing one left the server holding a document that no longer matched the buffer for the rest of its life: it kept reporting syntax errors against text nobody had written, and further editing only compounded the divergence rather than repairing it. A change that cannot be queued is now recorded rather than dropped - the editor re-sends the whole document as soon as the server has room again, and ignores anything the server derived from its stale copy in the meantime. Two related holes are closed with it: a `didOpen` that was refused used to be recorded as sent, which left the file invisible to that server for good, and closing a buffer never told the server at all - so a file closed with unsaved edits and reopened came back with the server still holding the text you discarded (#3038, reported by @thedadams).
...
* **Settings**: in the **Edit Item** dialog, `Delete` works in a text field again - it removes the character at the caret instead of doing nothing at all, which is how a scalar field like an LSP server's **Command** came to have a working `Backspace` and a dead `Delete`. Those fields now run on the same key table as every other text field in the editor, so word motion (`Ctrl`+`←`/`→`), word deletion (`Ctrl`+`Backspace`/`Delete`) and `Shift`+`Home`/`End` selection work there too; number fields in the same dialog gained `Delete`, `Home`/`End` and left/right caret motion, which had been silently ignored while typing (#2875, reported by @asukaminato0721).
...
* **Dim text in the integrated terminal is dim again** - a program's `ESC[2m` was dropped on the way through the integrated terminal, so text it meant to de-emphasise rendered at full brightness and read as ordinary text; bold, italic and underline on the same line all survived, which is what made it specific to SGR 2. The attribute is now carried out of the terminal grid alongside bold, italic and underline, both to the screen and into scrollback. Seen as Claude Code's input placeholder and input-box borders, which are drawn with a bare `ESC[2m`, looking like text the user had typed (#3123, reported by @fiatcode-gh).
...
* **Quitting no longer panics while a plugin's off-loop work is still running** - closing the editor while a plugin's `grepProject` or a diff-baseline load was in flight could take down the `editor-async` thread with `Cannot drop a runtime in a context where blocking is not allowed`. The background task held a share of the editor's tokio runtime, so when quitting released the editor's own share the task became the runtime's last owner and tried to shut the runtime down from inside itself. Off-loop plugin work now borrows the runtime rather than owning it, so the shutdown always happens on the editor thread. Most visible on a loaded or CPU-limited machine, where that work is still running by the time you quit.
...
* **Git and review commands are grouped under two prefixes** - the palette had `Review Diff`, `Stop Review Diff`, `Review Range`, `Review Stash` and `Review: Add Comment` scattered across four shapes, and `Review PR Branch` sitting among them while opening no review session at all: it lists the commits in `base..HEAD` with `git show` for the selected one, which is a git log scoped to a branch. Everything that opens the code review tool now starts with **`Review Diff: `** (`Stop`, `Refresh`, `Range (Commit or Branch)`, `Stash`, `Add Comment`, `Edit Note`, `Export to Markdown`, `Export to JSON`), and everything that browses commits starts with **`Git Log: `** - `Current File` (was `Git Log (Current File)`) joined by `PR Branch`, `Close PR Branch` and `Refresh PR Branch`, the former `Review PR Branch` and its panel commands. Each renamed command carries its old name in its description, and the palette matches descriptions too, so typing the name you know still finds it. The branch panel's tab is now `*Git Log: main..HEAD*`, and the dashboard's clickable `review branch` row is `branch log`. To review a branch rather than browse it, `Review Diff: Range` takes `main..HEAD` and flattens it into one diff.
...
* **A daemon speaks the language you configured** - `locale` in `config.json` chose the UI language for `fresh file.py`, but a session daemon came up in English regardless: the config reached the i18n layer in one place that daemon mode returns before, so the daemon fell back to whatever `LC_ALL`/`LC_MESSAGES`/`LANG` said. A daemon now reads the same locale the standalone editor does, and the flags typed on the client's command line are carried across to the daemon it starts: `--locale`, and `--config <path>` so that starting a session with an alternate config file gets a session that actually reads it. Attaching to an already-running daemon leaves both alone — it is already serving other terminals (#3149, reported by @kirinriki7777-sys).
* **Ctrl+C and `kill` now end the editor promptly and reliably** - `SIGINT`/`SIGTERM` were handled by doing the whole diagnostic dump inside the signal handler: allocating, taking locks, writing to the log, calling into the plugin runtime, and asking every thread to unwind itself. A signal handler runs on whichever thread the kernel interrupted, wherever that thread happened to be - quite possibly inside the allocator, holding its lock - so the dump deadlocked against it, and a failed lock assertion inside a C-ABI handler aborts the process outright. In practice a `pkill fresh` ended in `SIGABRT`, `SIGSEGV`, or occasionally nothing at all, instead of a clean exit. The handler now does one thing, the one thing POSIX allows: it hands the signal to a thread that was already waiting, and returns. Everything else happens on that thread, where it is safe, and a watchdog ends the process on a deadline whether or not the report finishes - so a terminating signal always terminates. A second Ctrl+C is now an immediate escape rather than being ignored. **The per-thread backtrace dump is off by default**, because capturing a backtrace allocates and there is no way to do that inside a signal handler safely: set `FRESH_SIGNAL_BACKTRACES=1` to get it back when you are chasing a hang. The running-JavaScript report, which is the part that costs nothing, still happens on every terminating signal.
* **Piped-in text no longer leaves a copy in `/tmp` for a day** - `… | fresh -` spools stdin to a scratch file so a large input can be paged in lazily, and nothing deleted it on the way out; `/tmp/fresh-stdin-<pid>.tmp` waited for the 24-hour sweep of stale logs, which is a lot of disk to leave behind after piping in a decompressed log. The scratch file now has no name at all: it is unlinked the moment it is created and held open instead, so the kernel reclaims it when fresh's last handle closes. That covers every way the editor can end, including the ones no cleanup code could ever have caught — `SIGKILL`, a segfault, a power cut — and it is never visible in `/tmp` even while the editor is running (#3134, reported by @Korkman).
