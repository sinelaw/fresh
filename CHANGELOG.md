...
* **Text fades in as you scroll it into view** - a row the scroll reveals arrives dimmed and climbs to its normal colour over the next few rows of travel, so the edge the content enters through reads as motion instead of a jump. The band is a fixed few rows deep however fast you scroll, and the rows a scroll merely shifted keep their colours; text sitting on a selection or the current-line highlight rises out of that band rather than out of the editor background. Off with `editor.scroll_fade_animation`, and with the master `editor.animations` toggle.
