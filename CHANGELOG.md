...
* **Piped-in text no longer leaves a copy in `/tmp` for a day** - `… | fresh -` spools stdin to `/tmp/fresh-stdin-<pid>.tmp` so a large input can be paged in lazily, and nothing deleted it on the way out; the file waited for the 24-hour sweep of stale logs, which is a lot of disk to leave behind after piping in a decompressed log. The file is now removed when fresh exits — quitting, an early failure, `pkill`, or closing the terminal window (SIGHUP), which used to leak it like everything else. A `SIGKILL` or a segfault still can't be caught by the process itself, and the 24-hour sweep remains the backstop for those (#3134, reported by @Korkman).
