...
* **`curl ... | install.sh` no longer fails behind a shared IP** - the installer needed a GitHub API call just to find the download URL, and that API allows 60 requests per hour per source IP, so from a NAT, a CI runner, or an office network it answered `403` and the install died reporting `curl: (3) bad range in URL position 3` - which named curl's URL parser rather than the rate limit that actually stopped it. The universal build is now fetched from `/releases/latest/download`, which takes no API call and has no such limit, with the API kept as a fallback and `GITHUB_TOKEN`/`GH_TOKEN` honoured when set. A failed lookup can also no longer be mistaken for a successful one: diagnostics go to stderr instead of stdout, so an error message can never be captured and used in place of the URL it was reporting the absence of.
