...
* **Enter on the blank last line of a text file no longer indents to the block above** - the fallback indenter for files without a language copied the previous block's indent onto a blank last line, so the next character landed eight spaces in. A blank line now keeps the cursor's own column: none at column 1, the full indent when Enter is pressed at the end of whitespace a previous Enter inserted. Only files with no language are affected; code keeps indenting a blank line inside an unclosed block (#3165).
