...
* **Typing over an auto-closed bracket now works with multiple cursors** - with more than one cursor live, typing the `)` that auto-close had already put there did nothing at all: every cursor stayed *inside* the parens, so the `;` typed after it landed there too and `a.open();` came out as `a.open(;)` on every line. Typing a closing delimiter that is already in front of the cursor is a skip-over - it steps the cursor past the character rather than editing anything - and the multi-cursor path applied keystrokes through a bulk edit that refuses an event list containing no edits, so a keystroke whose whole effect was to move cursors was discarded for every cursor at once. It now takes the same batch every other cursor-only action already used (#3125, reported by @X-Ryl669).
