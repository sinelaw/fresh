...
* **"Review PR Branch" is now "PR Branch Log"** - that command never opened a review session: it lists the commits in `base..HEAD` with `git show` for the selected one, which is a git log scoped to a branch. Renamed (along with `PR Branch Log: Close` and `PR Branch Log: Refresh`) so everything called *Review* in the palette opens the code review tool. To actually review a branch, `Review Range (Commit or Branch)` takes `main..HEAD` and flattens it into one reviewable diff. The dashboard's clickable `review branch` row is now `branch log`, and the panel that opens is titled `*Branch Log main..HEAD*`.
