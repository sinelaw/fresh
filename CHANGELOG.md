...
* **Mouse wheel scroll amount is configurable** - `editor.mouse_wheel_scroll_lines` sets how many lines one notch of the wheel moves the view, and now defaults to **1** rather than 3, so a scroll runs continuously instead of jumping the view in blocks. Shift+wheel still pans sideways by columns.
