...
* **The Welcome screen gained a Contents outline in the sidebar** — the same place Markdown files get theirs — plus a first-screen row that opens the file explorer, a terminal, a split or Settings, and a section on **Review Diff** with the keys it actually binds. `/` and `0` are no longer bound by the page: the finder's field takes focus by having the caret on it, so `/` is a path separator again.
