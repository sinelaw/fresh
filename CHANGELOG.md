...
* **Enter on the blank last line of a text file no longer indents to the block above** - in a plain-text file ending in an indented block and one empty line, Enter on that empty line put the cursor at `Ln 6, Col 9` and the next character eight spaces in. The fallback indenter for files without a language scanned backwards past the blank line and copied the previous block's indent; the look-ahead that stops it when a less-indented line follows (#1425) has nothing to look at when the blank line is the last one. On a blank line the new line now keeps the cursor's own column - none at column 1, the full indent when Enter is pressed at the end of the whitespace a previous Enter inserted, so typing an indented block is unchanged. Only files Fresh has no language for (plain text, unknown extensions) get this; a code file keeps indenting a blank line at the end of an unclosed block, whether tree-sitter cannot yet parse it or the language has no grammar at all (#3165).
