...
* **A click ends multi-cursor editing** - with several cursors live (`Ctrl+D`, `Ctrl+Alt+Down`), a plain click moved one of them to the clicked spot and kept the rest, which may well have been scrolled out of view, so the status bar still read `2 cursors` and the next keystroke edited every one of them. A plain click now collapses to a single cursor at the click, as in VS Code, Sublime and Zed; Shift-click (or Ctrl-click, for terminals that intercept Shift) still extends the selection and leaves the cursor set alone. Like `Esc`, the collapse is an undo step: `Ctrl+Z` after the click brings the cursors back (#3125, reported by @X-Ryl669).
