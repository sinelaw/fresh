...
* **Enter on the blank last line of a text file no longer indents to the block above** - in a plain-text file ending in an indented block and one empty line, Enter on that empty line put the cursor at `Ln 6, Col 9` and the next character eight spaces in. The fallback indenter for files without a language scanned backwards past the blank line and copied the previous block's indent; the look-ahead that stops it when a less-indented line follows (#1425) has nothing to look at when the blank line is the last one. On a blank line the new line now keeps the cursor's own column - none at column 1, the full indent when Enter is pressed at the end of the whitespace a previous Enter inserted, so typing an indented block is unchanged. This applies to files with no language and to languages without indent rules of their own (HTML, YAML, TOML, Markdown, XML, SQL, Makefile, ...); one consequence there is that Enter at column 1 of an *already blank* line under a line ending in `:` or `{` no longer adds a level, while Enter at the end of that `key:` line still does. Code that tree-sitter cannot yet parse keeps indenting a blank line at the end of an unclosed block (#3165).
