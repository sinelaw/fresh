...
* **The Welcome screen's caret and its focus stay on the same thing**: Tab brings the caret to the control it focuses, moving the caret onto a paragraph clears focus so `Enter` does nothing, and landing on a control — the file finder's field included — focuses it.
