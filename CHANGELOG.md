...
* **Integrated terminal fidelity**
  * **Dim text is dim again** - a program's `ESC[2m` was dropped on the way through the integrated terminal, so text it meant to de-emphasise rendered at full brightness and read as ordinary text; bold, italic and underline on the same line all survived, which is what made it specific to SGR 2. The attribute is now carried out of the terminal grid like every other one, both to the screen and into scrollback. Seen as Claude Code's input placeholder and input-box borders, which are drawn with a bare `ESC[2m`, looking like text the user had typed (#3123, reported by @fiatcode-gh).
  * **Shift+Enter reaches the program inside** - Enter's escape sequence is a bare `\r` with nowhere to put a modifier, so Shift+Enter, Ctrl+Enter and Shift+Enter in the `modifyOtherKeys` encoding all arrived at the program inside as a plain Enter. A program that asks for the kitty keyboard protocol (`CSI > 1 u`) now gets `CSI 13 ; <mods> u` instead - the encoding kitty itself sends - so the two are distinguishable. The request was previously discarded unread by the emulator, so fresh could not tell such a program from any other. Programs that never ask keep the legacy encoding exactly as before. Concretely: multi-line input in Claude Code, which submits on Enter and inserts a newline on Shift+Enter, works in the integrated terminal without reaching for Alt+Enter (#3124, reported by @fiatcode-gh).
