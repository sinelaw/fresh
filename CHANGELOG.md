...
* **`echo 123 | fresh` opens the pipe instead of hanging** - piping into `fresh` with no file and no `-` brought up an editor that took no input at all, spun on a full CPU core, and could not even be quit with Ctrl+Q. A pipe with nothing else to open now means `fresh -`; with a file to open it is left alone, but the keyboard works either way (#3252)
