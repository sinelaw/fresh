...
// =============================================================================
// customizations.fresh.plugins (spec §7)
// =============================================================================
//
// The Dev Container spec has `customizations.<tool>` for tool-specific
// extension points (`customizations.vscode.extensions` being the
// canonical example). VS Code extensions don't apply to Fresh — different
// plugin model — but the *shape* of the feature is worth mirroring under
// a Fresh-specific `customizations.fresh.plugins` namespace so config
// authors can ship plugins scoped to the attached container.
//
// Entries are plugin paths resolved against the workspace cwd (relative)
// or used as-is (absolute). They load after the container authority is
// active, so customization plugins see the container's filesystem via
// `editor.loadPlugin`. Failures log via `editor.debug` per Q-E1; a
// surfaced action popup can follow if the silent-fail shows up in
// practice.
//
// The handler fires on `plugins_loaded` — same as
// `devcontainer_maybe_show_attach_prompt` — so we see the freshly
// installed authority. Loads are deduped across restart cycles via
// `loadedCustomizationPaths`.

const loadedCustomizationPaths = new Set<string>();

interface FreshCustomizations {
  plugins?: string[];
}

function readFreshPluginPaths(): string[] {
  const freshBlock = (config?.customizations as Record<string, unknown> | undefined)?.fresh;
  if (!freshBlock || typeof freshBlock !== "object") return [];
  const entries = (freshBlock as FreshCustomizations).plugins;
  if (!Array.isArray(entries)) return [];
  return entries.filter((e): e is string => typeof e === "string" && e.length > 0);
}

/// Resolve a customization plugin entry to an absolute path. Relative
/// entries join against the workspace cwd — inside the container the
/// workspace is bind-mounted, so host and container paths coincide.
/// Absolute entries pass through unchanged.
function resolveCustomizationPath(entry: string): string {
  if (entry.startsWith("/")) return entry;
  return editor.pathJoin(editor.getCwd(), entry);
}

async function loadCustomizationPlugin(entry: string): Promise<void> {
  const resolved = resolveCustomizationPath(entry);
  if (loadedCustomizationPaths.has(resolved)) {
    editor.debug(
      `devcontainer: customization plugin already loaded, skipping: ${resolved}`,
    );
    return;
  }
  try {
    const ok = await editor.loadPlugin(resolved);
    if (ok) {
      loadedCustomizationPaths.add(resolved);
      editor.debug(`devcontainer: loaded customization plugin: ${resolved}`);
    } else {
      editor.debug(
        `devcontainer: customization plugin load returned false: ${resolved}`,
      );
    }
  } catch (e) {
    editor.debug(
      `devcontainer: customization plugin load threw: ${resolved}: ${String(e)}`,
    );
  }
}

/// On `plugins_loaded`, iterate `customizations.fresh.plugins` and load
/// each entry when the active authority is a container. We gate on the
/// authority label so running Fresh locally against a workspace with a
/// devcontainer.json doesn't auto-load container-scoped plugins.
async function devcontainer_load_customization_plugins(): Promise<void> {
  if (!config) return;
  const authorityLabel = editor.getAuthorityLabel();
  if (!authorityLabel.startsWith("Container:")) return;

  const entries = readFreshPluginPaths();
  if (entries.length === 0) return;

  for (const entry of entries) {
    await loadCustomizationPlugin(entry);
  }
}
registerHandler(
  "devcontainer_load_customization_plugins",
  devcontainer_load_customization_plugins,
);
editor.on("plugins_loaded", "devcontainer_load_customization_plugins");
