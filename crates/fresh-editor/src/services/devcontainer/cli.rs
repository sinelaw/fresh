...
impl DevcontainerCli {
    ...
    /// Read the devcontainer configuration without starting a container.
    ///
    /// Returns the parsed JSON configuration.
    pub async fn read_configuration(
        &self,
        workspace_folder: &Path,
    ) -> Result<serde_json::Value, DevcontainerError> {
        let output = Command::new(&self.cli_path)
            .args([
                "read-configuration",
                "--workspace-folder",
        ...
        serde_json::from_str(&stdout)
            .map_err(|e| DevcontainerError::ParseError(format!("{}: {}", e, stdout)))
    }

    /// Execute a command inside the running devcontainer.
    ///
    /// Uses `devcontainer exec` to run a command in the container context.
    pub async fn exec(
        &self,
        workspace_folder: &Path,
        command: &[&str],
