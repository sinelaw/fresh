...
pub enum UiFact {
    ...
    /// document's — or `None` off the run, where `above` says which of its two
    /// ends the drag reaches. Transient: it moves a selection, never the
    /// routing.
    WidgetProseDrag {
        slot: super::widgets::Slot,
        widget: String,
        byte: Option<usize>,
        /// The pointer is above the run's first row rather than past its last.
        /// Off the run is one of its ends and only the description knows
        /// which: the applier has the document, not where the pointer went.
        above: bool,
    },
    ...
    ///
    /// The crumb's own node answered the press, so this names the byte the
    /// caret goes to. There is no cell here and nothing to resolve: which
    /// crumb was hit is what the tree already decided by placing it.
    PaneBreadcrumbPress {
        pane: LeafId,
        position: usize,
    },
    ...
    /// A press outside a keyed `Overlay`. The plugin is told which one and
    /// decides what closing means. The press still reaches what it was
    /// aimed at, so a click on a row with the dock's `⋯` menu up both
    /// closes the menu and selects the row.
