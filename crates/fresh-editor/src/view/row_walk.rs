...
//! viewport and the renderer in different coordinates, which is worse. Of the
//! plugins that emit them, only `flash` is reachable on the buffers this module
//! serves: soft breaks are Compose-mode only, and the markdown conceals need a
//! markdown buffer. [`Viewport::top_visual_row_source_byte`] already declines to
//! answer where any of them intersect its span.
