...
mod tests {
    ...
    /// What the runtime's own renderer paints, for a call site that declares
    /// no styles of its own.
    fn ladder(kind: ButtonKind, bare: bool, focused: bool, hovered: bool, disabled: bool) -> Look {
        let entry = match bare {
            true => crate::widgets::render_bare_button(
                "Label", focused, kind, disabled, None, hovered, None,
            ),
            false => crate::widgets::render_button(
                "Label", focused, kind, disabled, false, None, hovered, None,
            ),
        };
        from_overlay(entry.inline_overlays.first().map(|o| &o.style))
    }

    fn states() -> impl Iterator<Item = (ButtonKind, bool, bool, bool, bool)> {
        [ButtonKind::Normal, ButtonKind::Primary, ButtonKind::Danger]
            .into_iter()
            .flat_map(|kind| {
                [false, true].into_iter().flat_map(move |bare| {
                    [false, true].into_iter().flat_map(move |focused| {
                        [false, true].into_iter().flat_map(move |hovered| {
                            [false, true]
                                .into_iter()
                                .map(move |disabled| (kind, bare, focused, hovered, disabled))
                        })
                    })
                })
            })
    ...
    /// **The claim the migration rests on.** Every state a button can be in
    /// resolves, through the class table, to the ink the hand-written ladder
    /// produced — so moving the call site over is a change of spelling and not
    /// of appearance. Forty-eight combinations, small enough to check
    /// exhaustively and large enough that every `..base` spread the ladder
    /// relied on is exercised.
    #[test]
    fn the_class_table_reproduces_the_button_ladder_exactly() {
        for (kind, bare, focused, hovered, disabled) in states() {
            let classes = button_classes(kind, bare, focused, hovered, disabled);
            assert_eq!(
                from_decl(&cascade(&classes).ink),
                ladder(kind, bare, focused, hovered, disabled),
                "{kind:?} bare={bare} focused={focused} hovered={hovered} \
                 disabled={disabled} spelled {classes:?}"
            );
        }
    }

    /// **The two renderers agree on how wide a button is.** The runtime writes
    /// the frame into its text; the shell reserves columns and lets the fold
    /// draw the glyphs there. Both read `Frame::BUTTON`, and this is what says
    /// the two readings of it come out the same — the check the old
    /// `FRAMED_BUTTON_CHROME_COLS` constant stood in for, now made against the
    /// rendered string rather than against another number.
    #[test]
    fn the_box_is_as_wide_as_the_text_the_runtime_writes() {
        let rendered = crate::widgets::render_button(
            "Go",
            false,
            ButtonKind::Normal,
            false,
        ...
        assert_eq!(rendered.text, "[ Go ]");

        let framed = cascade(&button_classes(
            ButtonKind::Normal,
            false,
            false,
            false,
        ...
        assert_eq!(
            usize::from(framed.reserved_x()) * 2 + "Go".chars().count(),
            rendered.text.chars().count(),
            "the columns the box reserves are the columns the frame occupies"
        );

        // A bare affordance reserves nothing, because it draws no frame.
        let bare = cascade(&button_classes(
            ButtonKind::Normal,
            true,
            false,
            false,
        ...
        assert_eq!(bare.reserved_x(), 0);
        assert!(bare.border.is_none());
    }
