...
/// True for paths inside a `.git` directory (e.g. `COMMIT_EDITMSG`,
/// `MERGE_MSG`, `TAG_EDITMSG`, rebase todo lists). Git regenerates these
/// files with fresh content on every operation, so persisted per-file
/// cursor/scroll state is always stale: restoring a byte offset saved for
/// the *previous* commit message lands the cursor mid-way into the new
/// content (typically inside git's comment block) and typing corrupts it
/// (issue #2761). Such files are excluded from per-file state persistence
/// on both the save and the load side (load-side too, so state files
/// written by older builds are ignored rather than applied).
///
/// A component match (not just a basename prefix) also covers linked
/// worktrees, whose git dir lives under `<main>/.git/worktrees/<name>/`.
...
impl PersistedFileWorkspace {
    ...
    pub fn load(path: &Path) -> Option<SerializedFileState> {
        // Git-internal files are regenerated per operation; never restore
        // state for them, even if an older build persisted some (#2761).
    ...
    pub fn save(path: &Path, state: SerializedFileState) {
        // Git-internal files are regenerated per operation; persisting
        // their cursor/scroll state is never useful (#2761).
        if is_git_internal_path(path) {
            return;
        }
        let state_path = match Self::state_file_path(path) {
...
mod tests {
    ...
    fn test_is_git_internal_path() {
        // Git-internal (regenerated, ephemeral) files.
    ...
    /// Regression for #2761: the per-file global state store must neither
    /// persist nor restore state for files inside `.git/` — they are
    /// regenerated by git for every operation, so a restored byte offset
    ...
    fn test_persisted_file_workspace_skips_git_internal_files() {
        ...
        // Save must be a no-op for a git-internal path…
