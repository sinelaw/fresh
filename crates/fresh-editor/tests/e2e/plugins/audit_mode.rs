...
/// to the status bar.
#[test]
fn test_branch_log_status_messages_are_translated() {
    init_tracing_from_env();
    let repo = GitTestRepo::new();
    setup_audit_mode_plugin(&repo);
    ...
    repo.create_file("a.txt", "base\nfeature\n");
    repo.git_add_all();
    repo.git_commit("add feature line");
    ...
    harness
        ...
        .unwrap();
    // Accept the default base ref (master).
    harness
        .send_key(KeyCode::Enter, KeyModifiers::NONE)
        .unwrap();
    harness.wait_for_prompt_closed().unwrap();

    // The commit list renders, and the ready status names the range.
    harness
        .wait_until(|h| {
            let s = h.screen_to_string();
            s.contains("add feature line") && s.contains("master..HEAD")
        })
        .unwrap();

    // q on the log panel closes the view. Wait for the panel itself to go
    // away — that happens either way — and then read the status message it
    // left behind, so a regression fails the assert instead of hanging here.
    harness
        .send_key(KeyCode::Char('q'), KeyModifiers::NONE)
        .unwrap();
    harness
    ...
    assert!(
        ...
        "Closing the branch log should print the translated message, not the \
         raw `status.` key. Screen:\n{}",
        screen
    );
    assert!(
        !screen.contains("status."),
        "The status bar should never show a raw i18n key. Screen:\n{}",
        screen
    );
}
