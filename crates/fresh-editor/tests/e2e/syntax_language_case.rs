...
/// Test that user-configured languages without a matching syntect grammar
/// appear in the language selector and can be selected.
///
/// Issue: When configuring a custom language like "fish" with grammar "fish"
/// (which doesn't exist in syntect), the language selector:
/// 1. Didn't show "fish" as an option
/// 2. Incorrectly highlighted "bash" as the current language
/// 3. Couldn't be used to set the language back to "fish"
#[test]
fn test_config_only_language_appears_in_selector() {
    ...
    // Step 1: Verify the language is "fish" (not bash)
    let language = harness.editor().active_state().language.clone();
    eprintln!("Language detected for .fish file: '{}'", language);
    assert_eq!(
        language, "fish",
        "Opening a .fish file with fish config should set language to 'fish'"
    );

    // Step 2: Set language to Plain Text first
    run_command(&mut harness, "Set Language");
    harness.wait_for_prompt().unwrap();
    ...
    let language = harness.editor().active_state().language.clone();
    assert_eq!(language, "text", "Language should now be 'text'");

    // Step 6: Now set it back to fish via the selector
    run_command(&mut harness, "Set Language");
    harness.wait_for_prompt().unwrap();
    ...
    let language = harness.editor().active_state().language.clone();
    eprintln!("Language after selecting fish from selector: '{}'", language);
    assert_eq!(
        language, "fish",
        "Selecting 'fish' from the language selector should set language to 'fish'"
    );
}
