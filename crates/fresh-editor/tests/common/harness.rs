...
impl EditorTestHarness {
    ...
    pub fn send_key(&mut self, code: KeyCode, modifiers: KeyModifiers) -> AnyhowResult<()> {
        ...
        // Drain async work this key kicked off before returning.
        // The editor dispatches mode bindings (e.g. `Home` ->
        // `search_replace_home`) as plugin actions that run on a
        // separate plugin thread via `execute_action_async` — a
        // single `process_async_messages` call only sees results
        // that have already arrived in the queue, not work still
        // in-flight on the plugin thread.
        //
        // Without this drain, a test sequence like
        // `send_key(Home); send_key(Right, SHIFT)` could observe
        // the synchronous-bypass `Shift+Right` run *before* the
        // mode-bound `Home` finishes — leaving the selection
        // anchored at the wrong cursor position. That's the
        // Windows CI flake source for the Stage 1 widget Text
        // tests. Production `handle_key` runs in a frame loop
        // where multiple frames naturally pump everything; tests
        // do not get that for free.
    ...
    pub fn send_key_repeat(
        ...
        // Drain any async plugin actions and bridge messages
        // before returning — see `send_key` for the rationale.
        self.drain_async_work();
    ...
    /// Drain async plugin work to a fixed point. Called by
    /// `send_key` / `type_text` / `send_key_repeat` so each
    /// keypress's full ripple effect (plugin actions on the
    /// plugin thread, follow-on `WidgetCommand` messages on the
    /// async bridge, the host's response to those commands) has
    /// fully applied by the time the call returns. Without this,
    /// tests can race when interleaving keys whose mode binding
    /// dispatches a plugin action (e.g. `Home`,  `Backspace`,
