...
pub fn remove_json_pointer(root: &mut Value, pointer: &str) {
    if pointer.is_empty() || pointer == "/" {
        return;
    }
...
impl DirectoryContext {
    ...
    pub fn project_state_dir(&self, working_dir: &std::path::Path) -> std::path::PathBuf {
        ...
        self.workspaces_dir()
            .join(crate::path_encode::encode_path_for_filename(&canonical))
    }
    ...
    pub fn terminal_dir_for(&self, working_dir: &std::path::Path) -> std::path::PathBuf {
        let encoded = crate::path_encode::encode_path_for_filename(working_dir);
