...
use fresh_core::api::{
    ButtonKind, DualListOption, HintEntry, LabelAlign, OverlayColorSpec, OverlayOptions, TreeNode,
    WidgetSpec,
};
...
pub struct RenderContext<'a> {
    ...
    /// Which way form controls align their labels in the shared
    /// `label_width` column. Panel-wide, like `marker_gutter`: a column
    /// aligns as one, so no control decides this alone.
    pub label_align: LabelAlign,
    /// How many columns left of this widget's row a float it hangs off
    /// starts: `2` inside a `LabeledSection` (the popup re-adds the `│ `
    /// chrome it paints over), `0` at the panel edge. A `Text` field's
    /// completion list uses it to put its candidates under the value.
    pub popup_escape: u32,
...
impl RenderContext<'_> {
    ...
    pub fn is_focused(&self, key: Option<&str>) -> bool {
    ...
    /// hover key.
    pub fn is_hovered(&self, key: Option<&str>) -> bool {
    ...
    /// one.
    pub fn is_row_hovered(&self, key: Option<&str>, item_key: &str) -> bool {
...
pub fn mark_list_row_selected(entry: &mut TextPropertyEntry) {
...
/// display columns.
pub fn render_section_bottom_border(total_cols: usize) -> TextPropertyEntry {
    let mut text = String::new();
    text.push('╰');
    for _ in 0..total_cols.saturating_sub(2) {
        text.push('─');
...
pub fn render_label(
    ...
    let mut row = String::with_capacity(marker.len() + indent + text.len());
    row.push_str(marker);
    row.extend(std::iter::repeat_n(' ', indent));
    let start = row.len();
    row.push_str(text);
    let end = row.len();
    let mut overlays = Vec::new();
...
pub fn render_radio(
    ...
    for (i, opt) in options.iter().enumerate() {
        if i > 0 {
            text.push_str(RADIO_GAP);
        }
        let start = text.len();
        let on = i as i32 == selected_index;
        text.push_str(if on { "(•)" } else { "( )" });
...
pub fn tree_max_scroll(heights: &[u32], visible_rows: u32) -> u32 {
...
pub fn snap_down_to_char_boundary(s: &str, idx: usize) -> usize {
