...
impl Resolved {
    ...
    pub fn body_rows(&self, visible_rows: u32) -> usize {
        self.available
            .len()
...
pub fn header_row(st: &Resolved, col_w: usize) -> TextPropertyEntry {
...
pub fn body_row(options: &[DualListOption], st: &Resolved, i: usize, col_w: usize) -> Row {
...
pub fn label_row(label: &str) -> Option<TextPropertyEntry> {
    if label.is_empty() {
        return None;
    }
...
pub fn hint_row(hint: &str) -> Option<TextPropertyEntry> {
    if hint.is_empty() {
        return None;
    }
