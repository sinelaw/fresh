...
mod tests {
    ...
    fn en(pairs: &[(&str, &str)]) -> HashMap<String, HashMap<String, String>> {
        let inner = pairs
            .iter()
            .map(|(k, v)| (k.to_string(), v.to_string()))
            .collect();
        HashMap::from([("en".to_string(), inner)])
    }

    #[test]
    fn translates_interpolates_and_unregisters() {
        register_plugin_strings("p-basic", en(&[("hi", "Hi, %{who}")]));

        let args = HashMap::from([("who".to_string(), "you".to_string())]);
        assert_eq!(translate_plugin_string("p-basic", "hi", &args), "Hi, you");

        // Unknown plugin and unknown key both degrade to the key itself.
        assert_eq!(translate_plugin_string("p-absent", "hi", &args), "hi");
        assert_eq!(translate_plugin_string("p-basic", "bye", &args), "bye");

        unregister_plugin_strings("p-basic");
        assert_eq!(translate_plugin_string("p-basic", "hi", &args), "hi");
    }

    #[test]
    fn missing_arguments_leave_the_placeholder() {
        register_plugin_strings("p-args", en(&[("hi", "Hi, %{who}")]));
        assert_eq!(
            translate_plugin_string("p-args", "hi", &HashMap::new()),
            "Hi, %{who}"
        );
        unregister_plugin_strings("p-args");
    }
}
