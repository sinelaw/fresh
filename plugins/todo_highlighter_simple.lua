-- TODO Highlighter Plugin (SIMPLIFIED with marker-based overlays)
-- Highlights TODO, FIXME, HACK, NOTE, XXX, and BUG comments in the current buffer
--
-- KEY IMPROVEMENT: With marker-based overlays, we no longer need to:
-- 1. Re-scan the buffer on every edit (removed after-insert/after-delete hooks)
-- 2. Manually track and update overlay positions
-- 3. Clear and recreate all overlays after each keystroke
--
-- Overlays now automatically adjust their positions when text is inserted/deleted!
...
-- ============================================================================
-- The old version re-scanned the entire buffer on every edit:
--
-- editor.on("after-insert", function()
--     if highlighting_enabled then
--         highlight_keywords()  -- O(n) buffer scan on EVERY keystroke!
--     end
-- end)
--
