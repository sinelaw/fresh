...
- **ROOT-CAUSED (Run #62, master 64b75a64d, 4/4):** there is NO stored replay — the "pending command" is **bash history recall from leaked arrow keys**. Dismissing an ORCHESTRATOR modal (Run Agent dialog 4/4, Manage modal "Orchestrator: Open" 2/2; Settings clean 1/1) breaks the NEXT Alt+O ("Toggle Orchestrator Dock Focus" — a toggle): the dock renders its row highlighted (looks focused) but keyboard stays in the terminal PTY, so the "switch workspace" ↑ = bash previous-history = last command shown pending. Killer proof: two ↑s walk to the SECOND-newest command (replay can't). Recovery: a second Alt+O focuses the dock properly (⇒ verifying focus before pressing arrows DESTROYS the repro — why the owner's attempt passed; anti-Heisenbug rule posted). Controls: no-modal Alt+O clean 2/2, palette-only clean 1/1. Two comments posted (tighter recipe + scope addendum, 2026-09-03). PC-61b (first-char swallow) likely same family: focus where the UI says it isn't.
...
- **Run #62:** OWNER-CONFIRMED on master `cf626fc` (their comment pins the regression window: 0.4.10 release tarball clean → lost in the retained-mode series); **still reproduces at HEAD `64b75a64d`** (↓ dismissed popup + moved dock selection, 1/1). No further comment posted (owner's own repro stands).
