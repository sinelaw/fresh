...
## NextPane/PrevPane + pane/split cycling (Run #59b, v0.4.4 @ f545a75)
- **New in v0.4.4 (#2562, @masmu):** actions `next_pane`/`prev_pane` — "cycle through every split+tab pane as one flat list, distinct from `NextSplit`/`PrevSplit` and `NextWindow`/`PrevWindow`; landing on a terminal now also switches it into terminal mode." **NO default keybinding, NO palette command** (palette search `pane` → 0 hits) → reachable ONLY via a custom keybinding. Sibling `next_split`/`prev_split` DO have palette entries ("Next Split"/"Previous Split"; `prev_split` = `Alt+[`). → IMP-032 (discoverability, not a bug).
- **Split/pane commands (palette):** Split Horizontal, Split Vertical, Close Split, Decrease/Increase Split Size, Next Split, Previous Split (`Alt+[`), Toggle Maximize Split. **Split Vertical DUPLICATES the active buffer into the new split** (both panes show the SAME buffer/tab — shared, so a type-a-char focus oracle needs each split switched to a DISTINCT file first).
- **Editor total binding count = 947** in v0.4.4 (was 866 in v0.3.12). Custom `"keybindings":[{"key":"F6","action":"next_pane"},…]` load fine (Keybinding Editor shows `F6 next_pane … normal custom`; context `normal`). F-keys ARE delivered (F10 opens menu; F12 fires LSP).
- **HARNESS WALL (why next_pane runtime is unverified — for Run #60 (d)):** could not reliably detect which split/pane is focused in tmux. (1) yellow tab bg `48;5;226` = **preview/special-tab** marker, NOT focus. (2) hardware cursor (`#{cursor_x/y}`) did NOT move on split-focus change. (3) `Alt+[` / any `M-[` is **uninjectable** — tmux `send-keys M-[` emits the CSI introducer `ESC [`, parsed as an escape sequence, and corrupts the following keystrokes. Only clean oracle: **type a unique char into distinct-buffer splits and see which buffer's content changes** (needs both splits pre-loaded with different files). Standing case for IMP-009 (`--test-mode`).
- **#2197 (pyright) recheck recipe on 0.4.4 (STILL BROKEN):** `{"lsp":{"python":{"command":"pyright-langserver","args":["--stdio"],"enabled":true,"auto_start":true}}}`, launch on a `.py`, workspace auto-Trusted, `LSP (python) ready`/`LSP (on)`, 2 procs (python wrapper + node child) — hover/F12/diagnostic each time out after 30s (warnings-*.log), `position_encoding=None` in init log. pyright 1.1.408 now uses pull diagnostics (`registerCapability … ["textDocument/diagnostic"]`) and even that stalls → Fresh-side request/response stall.
## Embedded terminals basics→mouse-selection (Run #59, v0.4.4 @ f545a75ad)
- **Open commands (palette):** "Open Terminal" (new TAB in current split), "Open Terminal to the Right" (vertical split), "Open Terminal Below" (horizontal split), "Open Terminal in Utility Dock" (Alt+`). Terminal tabs auto-disambiguate duplicates in the same cwd as `<proj> 1`, `<proj> 2`.
- **Modes:** Ctrl+Space toggles live↔read-only (status "Terminal mode enabled" / "Terminal mode disabled - read only (Ctrl+Space to resume)"). Ctrl+Home/Ctrl+End jump to top/bottom of scrollback; Ctrl+F searches scrollback (highlights match bg `48;5;16`/`226`, "Found N match", moves cursor). **Ctrl+] (documented exit alias) is a NO-OP in plain tmux** — 0x1D forwarded to the shell (PC-59a/IMP-033).
- **PTY sizing:** a focused split reports `tput cols`x`lines` matching its split geometry (e.g. 108x21 in a quarter of 220×50).
- **Mouse text selection (NEW 0.4.4) — drivable via SGR harness:**
  - Plain drag on a LIVE terminal → drops to read-only scrollback + real selection (bg `48;5;17`, same as editor). Selection is half-open (release col exclusive).
  - Ctrl+C → status "Copied - terminal resumed" (copies AND returns to live). Paste into an editor buffer = the exact selected span.
  - Double-click = word select (stops at `-`/punct delimiters); triple-click = whole line; bare single click = focus only (stays live, no selection).
  - **Mouse forwarding:** a program that enabled DECSET 1000/1006 (`printf '\033[?1000h\033[?1006h'; cat -v`) RECEIVES plain-drag SGR bytes (translated to grid-LOCAL coords: screen row − terminal-top-offset). **Shift+drag (button+4: press `\e[<4;C;RM`, motion `\e[<36;C;RM`, release `\e[<4;C;Rm`) BYPASSES forwarding and selects** (drops to read-only, no bytes forwarded). Disable via `terminal.mouse_drag_selects=false`; legacy rule via `terminal.mouse_forwarding="alt_screen"` (both untested — A10.6/A10.7).
  - SGR recipes: left click `\e[<0;C;RM`+`\e[<0;C;Rm`; drag press `\e[<0;C;RM` / motion `\e[<32;C;RM` / release `\e[<0;C;Rm`; double/triple = 2/3 rapid press+release pairs at one coord. Terminal content has NO line-number gutter (col 1 = first char).
- **NextPane/PrevPane (#2562):** "Next Pane"/"Previous Pane" cycle every split+tab as a flat list; landing on a terminal ENTERS terminal mode (typed text reaches the shell). Distinct from Next/Previous Split.
- **Per-split scrollback (#2595):** "Split Vertical/Horizontal" shows the SAME terminal in two splits; each split keeps its OWN mode and scroll position. One split can sit frozen in scrollback@top while the other follows live output; the scrollback split does NOT auto-jump to end on new output (Automatic Scroll / `terminal.jump_to_end_on_output`).
- **Scrollback persistence:** stored per-workspace at `~/.local/share/fresh/terminals/<enc-root>/fresh-terminal-N.txt`; reloads even under `--no-restore` (PC-59b/IMP-034).

## Workspace orchestration: folders + co-tenant extraction (Run #59, v0.4.4 @ f545a75ad)
- **0.4.4 condensed dock toolbar (#2703):** `[ New Task… ▾ ] [ Search Tasks ]` + collapsible `[ ▸ Filters ]` (replaces the old row of filter toggles). Alt+O (`M-o`) toggles dock focus.
- **Workspace folders (#2703):** commands — "Orchestrator: Move to Folder…" opens a `[ ● Top level ] / [ New Folder… ]` popup. New Folder dialog: name field (default "New Folder") + `[v] Organize "<ws>" under this folder` checkbox + Cancel/Create. Result: `▼ ▤ <Name>  (N)` header with nested workspaces (`┃· <ws>`). **F2 on a folder header** → menu: **Rename… / New Subfolder… / Delete Folder** (Esc to close). Rename pre-fills the current name. **Delete Folder removes the folder and returns its workspaces to top level (workspaces preserved, not deleted).** Nested subfolders supported. Persists across restart in `~/.local/share/fresh/orchestrator/state/orchestrator.json`. Dock footer hints: `↑↓ switch · →← fold · Enter edit · F2 menu`.
- **Co-tenant extraction (#2735 area):** "Extract Tab to New Workspace" (builtin) moves the current tab into a new workspace over the SAME project root, labeled `<proj> (N)`. Multiple co-tenant windows coexist over one root, live-switchable with no restart. Storage keyed by durable stable_id: `~/.local/share/fresh/workspaces/<enc-root>.ws-<stable_id>-<n>.json` (durable-identity v3, 0.4.4).
- **KEY default filter:** the Orchestrator modal ("Orchestrator: Open") has **"Show empty/1-file workspaces (Alt+I)" OFF by default** — co-tenant windows holding ≤1 file are HIDDEN from BOTH the dock and the "all workspaces" selector until Alt+I. Also present: `[ + New Alt+N ]`, `Project: [All ▾ (Alt+P)]`, `[ ] Show all worktrees (Alt+T)`, `Filter [type/ ]`. Right panel = live preview of the selected workspace's tabs.
- **BUG #2735:** extracting a tab creates the co-tenant live, but on quit only the SOURCE workspace is persisted — the extracted co-tenant is dropped → lost on restart (2/2 clean repro). Contradicts the 0.4.4 "restore each co-tenant window" commit.
- **Harness:** palette file-open needs FILE mode (no leading `>`); a stray `>` = command mode (matches commands, not files) and `C-u` doesn't reliably clear it — backspace the whole buffer, or pass files as CLI args (`fresh f1 f2`) for reliable multi-tab setup.

## Restart an Exited Terminal in place (0.4.6 flagship) + tab-name-on-restart (Run #60, v0.4.10)
Docs: `docs/features/terminal.md` §Restarting an Exited Terminal. App-reference contract, all UI-verified:
- **Exit → dead terminal:** when the shell/process quits, the tab gains an `(exited)` suffix and the buffer becomes read-only scrollback frozen on the last screen (nothing appended). Status-bar **bottom-left** shows a clickable `⟳ Restart terminal` (named `⟳ Restart <cmd>` / `⟳ Resume claude` for known agents). Non-zero exit rides along: `⟳ Restart terminal (exit 3)`. Status message "Terminal N exited".
- **Restart 3 equivalent ways:** click the `⟳` indicator (SGR click works — it's the `{terminal_restart}` status element); palette **"Restart Terminal Process"** (desc "Respawn the exited process in this terminal buffer, resuming the agent session when there is one"); View → Terminal → Restart Terminal Process. Result: process back **in the same buffer**, tab drops `(exited)`, status "Terminal N restarted". The **old transcript is preserved in scrollback** (Ctrl+Space→Ctrl+Home shows it) and the **new session is appended below** it.
- **Running-terminal guard:** invoking restart on a LIVE terminal → status "Terminal process is still running", no restart (no indicator appears while live).
- **Editor-restart survival:** quit (Ctrl+Q) with a dead terminal, relaunch `fresh` (pure restore) → the dead terminal pane comes back with its transcript AND a working `⟳ Restart terminal (exit N)` indicator (the exit CODE survives the editor restart), **dead-on-purpose** (never silently re-runs — matters for agents/tokens). Ctrl+Q quits cleanly even with a live or dead terminal (no prompt).
- **Live-vs-dead at quit:** a LIVE-at-quit terminal restores LIVE (its process is present after restore, so it keeps its `bash — <cwd>` name and scrollback); only an EXITED-at-quit terminal restores dead. (Note: this conflicts a little with the §Persistence line "running processes are terminated" — in practice a live terminal came back live in v0.4.10.)
- **BUG #3074 (filed):** the descriptive tab NAME is NOT preserved for an exited-then-restored terminal — it shows generic `*Terminal N*` instead of `bash — <cwd>`, though terminal.md §Persistence + the 0.4.6 changelog say names survive restart. Name is re-derived from the live process each time (restart regains `bash — ...`). 2 dead terminals collapse to indistinguishable `*Terminal 0* 1`/`*Terminal 0* 2`.
- **#2735 FIXED (0.4.9):** Extract-Tab co-tenant now survives restart — BOTH `workspaces/<root>.ws-<stable_id>-N.json` are written on quit (was only the source), and relaunch restores both windows with their files. `--no-restore` now writes no session layout (only `workspaces/<root>/trust.json`); pure restore after a `--no-restore` session shows empty `[No Name]`.

### Harness (Run #60)
- Palette `C-u` clears the leading `>` too → drops into FILE mode. To get command mode reliably: Escape, then `C-p` (which prefills `>`), then type the command. Don't rely on `C-u`.
- Status-bar element click is drivable: text sits after " Trusted  Local  " (col ~18+); status bar row = pane height (50 for 220×50). SGR press `\e[<0;COL;50M` + release `\e[<0;COL;50m` via `send-keys -l`.
- `pkill -f "fresh-master/target/release/fresh"` SELF-MATCHES the running shell command → exit 144 (kills own subshell). Use `ps aux | grep "[f]resh-master/target/release/fresh" | wc -l` to check, or just Ctrl+Q + `tmux kill-session`.
- Extract-Tab moves focus INTO the new co-tenant window (shows the extracted tab); the source window keeps the remaining tabs. Orchestrator dock (Alt+O) lists both `<proj>` and `<proj> (2)`; Up/Down live-switches windows.
- In v0.4.10 the Orchestrator dock showed both 1-file co-tenants WITHOUT needing Alt+I (the default may have changed since 0.4.4, or extracted 1-file windows count differently) — re-confirm the Alt+I default if it matters.

## Run Agent / New Workspace unified dialog + input-replay + folders (Run #61, v0.4.10 @ 7bcc8ff6c retained-mode)
- **One dialog, two entry points:** palette "Run Agent…" (`ORCHESTRATOR :: Run Agent`) and "Orchestrator: New Workspace" open the SAME dialog; the `Launch in: [Current workspace|New workspace ▼]` switch morphs the title and field set live. Footer: `Tab next/accept  S-Tab prev  ←→ change option  ↑↓ suggest/history  Space toggle  Enter advance/act  ^Enter create  Esc close`.
- **Agent cycle:** terminal → claude → codex → opencode → aider → custom…. Options per agent: `[ ] Auto mode` for claude/codex/aider (NOT opencode); Start prompt for all real agents; `[v] Teach agent the Fresh CLI` ONLY in New-workspace Advanced and ONLY for claude/codex/opencode (removed for aider per scripting.md). Current-workspace mode has NO Advanced section at all.
- **custom…** auto-focuses the Agent Command field. New-workspace mode: Run in [Local][SSH][Kubernetes][Devcontainer], Project Path (default current root), Workspace Name (auto `<proj>-N`), `[ ▼/▶ Advanced… ]` (Enter toggles in place) w/ Agent Command/Teach/`[v] Create a new git worktree`/Checkout branch/New branch name; buttons `[ Create Workspace ][ Create in Background ][ Cancel ]`.
- **Create in Background is truly non-blocking:** current workspace untouched; worktree `~/.local/share/fresh/orchestrator/<enc-proj>/<name>` on auto-branch `<name>`; the agent (terminal) is already RUNNING in the worktree when first visited. Dock rows auto-name from their terminal.
- **DIALOG STATE IS STICKY (trap!):** Launch-in, Agent (incl. custom… + its command), Advanced expansion persist across opens and across BOTH palette entry points. A remembered custom command SILENTLY re-runs in every subsequently created workspace → fresh terminals show its output. **Looks exactly like cross-workspace scrollback contamination (#2828-shape) but is NOT — check the `Agent:` row before filing.** (Run #61 burned ~30 min on this near-false-positive; IMP-035.)
- **BUG #3136 recipe (input replay):** cmd in terminal → Run Agent dialog open+Esc → dock switch away+back → the cmd line re-injected as PENDING PTY input (Enter re-executes; C-u clears). One-shot per dialog-open; bounce-only and palette-only controls clean. Related 1× anomaly: palette query text appended into the dialog's Agent Command field on open.
- **BUG #3137 (regression):** Move-to-Folder popup (`● Top level`/`New Folder…`) ignores its footer keys — ↓ moves the DOCK selection behind it (= live workspace switch) and dismisses the popup; mouse click on popup rows works. #2694 (closed) documents ↑↓ used to work there. The F2 folder-header menu (Rename/New Subfolder/Delete Folder) and the New Folder dialog take keys CORRECTLY — only this popup lost its grab.
- **Folders (0.4.10):** F2 menu needs dock keyboard focus (`M-o`); a mouse click on a folder header toggles fold but does NOT hand the dock the keyboard. New Folder dialog: name + `[v] Organize "<ws>" under this folder` → creates nested `FolderA ▸ SubB ▸ ws`. Dock `[Search Tasks]` filter keeps folder ancestry visible for matches. SGR drag on rows does nothing (no drag-move).
- **Extract Tab on a TERMINAL tab:** co-tenant `<proj> (2)` over the SAME root; the PTY survives extraction live — cwd, transcript, auto-title all preserved; extracted workspace inherits Trusted. (No "root at shell cwd" behavior — that was only ever a test_plan hypothesis.)

### Harness lessons (Run #61)
- **Palette recency ranking:** after running a command once, a typed query may rank a DIFFERENT item first than before ("New Workspace" query → Extract Tab first). Capture the highlighted row before Enter, or query by unique description text (`Spawn a new editor`).
- **First-char swallow after focus transitions:** 4 organic hits (first char of typed text eaten right after click-into-terminal / workspace switch: `at -v`, `cho LEAKED`…) but 0/9 controlled trials — trigger involves prior palette/dialog state, likely #3136's input-routing family. RULE: after any click or workspace switch, verify the typed line before pressing Enter (or send+check a sacrificial char).
- With the File Explorer open the tab bar starts ~col 95 (explorer occupies ~41–93 on 220 cols) — recompute tab-click coordinates.
- Dock row visit = SGR click `\e[<0;5;ROW M`+`m`; status-bar `⟳ Restart terminal` still clickable; menu submenus open with `Right` and the FIRST item pre-selected (3×Down overshoots — count from the pre-selection).
- `cargo build --release` from a fresh worktree ≈ 12 min in-container; master can be FORCE-PUSHED (Run #60's HEAD vanished) — never `git log oldhead..HEAD`, use `--since=<last run date>` for the fix-commit scan.

## Orchestrator modal → Alt+O focus-toggle break (#3136 root cause) + retained-mode keyboard sweep (Run #62, v0.4.10 @ 64b75a64d)
- **#3136's "input replay" is NOT a replay.** Mechanism: dismissing an ORCHESTRATOR modal with Esc (Run Agent/New Workspace dialog, or the Manage modal = palette "Orchestrator: Open") leaves the dock flagged as focus owner. The next `Alt+O` ("Toggle Orchestrator Dock Focus" — a TOGGLE per its palette entry) therefore toggles focus AWAY → keyboard stays in the terminal PTY while the dock renders its selection highlight. The "workspace switch" ↑ lands in bash = previous-history = last command displayed as pending input; Enter re-executes; C-u clears; ↓ walks forward (can silently CLEAR the evidence). Two ↑s recall the SECOND-newest command — the discriminating experiment that kills the stored-replay theory. Second Alt+O recovers (one-shot). Settings modal does NOT trigger it. Scope: orchestrator plugin surfaces only.
- **Anti-Heisenbug rule:** verifying "is the dock really focused?" before pressing arrows (e.g. pressing Alt+O again, as the owner did) consumes the broken toggle and makes the bug unreproducible. To DETECT the state instead: send a sacrificial printable char — if it appends to the terminal's pending line, focus is the terminal regardless of what the dock highlight says.
- **Bash-history quirks as test oracles:** the history cursor persists across C-u within one prompt line — successive leaked ↑s across separate tests keep walking OLDER commands (observed E4 recalled long after AAA9/BBB9). Great for proving key-leak-into-PTY; confusing if forgotten.
- **Retained-mode popup keyboard grabs all PASS at 64b75a64d** except the known #3137 Move-to-Folder popup: New Task ▾ `create` dropdown, Alt+P project dropdown (M-p also auto-expands the Filters fold, revealing [ All ▾ ]/[ ] all worktrees/[v] show empty/[ Move… ][ Manage ]), Manage-modal workspace list (heavy ━┃ frame = selected row; buttons row highlights separately), tab context menu, tab-bar '+' popup. Printables in menus neither leak behind nor dismiss.
- **Welcome screen (new at this HEAD):** opens once as a `Welcome` tab on startup; ASCII logo + "WHAT BRINGS YOU HERE?" ladder cards `[1] JUST EDIT TEXT` `[2] CLASSIC IDE` `[3]` agents; number keys jump to LEVEL sections (3 = orchestrator card w/ LIVE workspace glyphs ● working ◐ idle ○ discovered + [ New workspace… ][ Run agent here… ][ Open the dock ] buttons); `[✓] Show this screen on startup` checkbox. Deep-dive pending (Run #63 (d)).
- **Scripting smoke:** `fresh --skill` prints the guide (script verbs incl. `script check`, `script api <query>`, `command run "<name>"`, `init reload`). From INSIDE an embedded terminal, `echo 'return editor.describeWorkspace().panes.map(p => p.name)' | /path/to/fresh --cmd script run` returns JSON against the RUNNING editor (`["terminal"]`) — no env setup needed.

### Harness lessons (Run #62)
- **CRITICAL: mouse-click columns must be CHARACTER-indexed, not byte-indexed.** `awk index()`/`grep -bo` count BYTES; tab bars and dock chrome contain multi-byte width-1 glyphs (`×` 2B, `—` 3B, `…` 3B, `▾` 3B, `│` 3B), so byte math overshoots the true screen column and SGR clicks miss silently. The '+' button "was dead" for 5 clicks until recomputed via `python3 len()` on the captured line (all these glyphs are width-1 → char index == display column). Same math applies to every click target right of any such glyph.
- `tmux send-keys -t S "long quoted string with spaces"` DOES deliver the string literally (used for the full `echo … | fresh --cmd script run` pipeline). The old spaces-are-separators rule applies to strings that could parse as key names; full-sentence args are safe.
- A click into a terminal pane can land the view at scrollback TOP (read-only-ish view) with the live prompt off-screen — always `C-End` (or check the last `#` line) before asserting on terminal content.
- Terminal scrollback nav keys (PageUp/arrows/C-Home) only act in SCROLLBACK mode (enter via C-Space, exit C-Space/C-End); in live mode they're forwarded to the PTY (bash: no-op / history). Docs structure says as much — don't file.
- tmux `C-Enter` is NOT deliverable (sent as plain Enter) — use Tab-to-button + Enter for dialogs' `^Enter create`.
- `fresh` binary must be invoked by full path inside its own embedded terminal (not on PATH in-container).
