...
## Run #62 — 2026-09-03 — #3136 root-caused after the owner's failed repro (tighter recipe delivered); #3137 re-verified at HEAD; retained-mode keyboard sweep PASS; welcome/scripting/F9/scrollback new coverage

**Preflight:** Synced state branch (up to date; origin fetched + HEAD compared per STATE-SYNC directive). Master ADVANCED 7bcc8ff6c → **64b75a64d** (still v0.4.10; ~30 commits: welcome-screen tab, sidebar-sections rework, bracketed-paste/symbolic-key-names input work, terminal transcript fixes). Lessons continuity OK (prose headings since ~R#35). Auth live (list_issues OK — 7 open agent issues). **Maintainer action found:** the owner's own automated agent commented on BOTH Run #61 issues: #3137 REPRODUCED on master `cf626fc` + pinned the regression window (0.4.10 release tarball clean → lost in the retained-mode series); #3136 could NOT be reproduced — explicit ask for a tighter recipe. No agent issue closed → no fixed-bug rechecks due. Built release binary from 64b75a64d (exit 0, ~13 min).

**Deliverable 1 — #3136 ROOT-CAUSED, tighter recipe + scope posted (2 comments).** On 64b75a64d, 4/4: terminal in a worktree workspace runs `echo BAIT` → `Ctrl+P` → Run Agent… → Enter → `Esc` → the NEXT `Alt+O` leaves keyboard focus in the terminal while the dock renders its selection highlight; the "switch" `↑` lands in bash = previous-history = the command sits at the prompt as pending input (the filed "replay"). Discriminating experiment: **two ↑s recall the second-newest command** (`echo AAA9` after `echo BBB9`) — impossible for a stored replay; plus probe-char appends to the pending line (focus provably terminal), `↓` = next-history silently CLEARS the evidence (explains the owner's clean-looking transcript). One-shot: the second `Alt+O` focuses the dock correctly → **verifying focus before pressing arrows destroys the repro** — exactly what the owner's careful attempt did (anti-Heisenbug rule posted). Scope: Manage modal ("Orchestrator: Open") triggers it too 2/2; Settings modal clean 1/1; no-modal control 2/2 clean; palette-only 1/1 clean ⇒ orchestrator modals specifically. Bonus mechanism hint posted: palette names Alt+O "**Toggle** Orchestrator Dock Focus" — modal teardown plausibly leaves the dock flagged as focus owner so the next toggle moves focus *away*. Comments 5531643294 + 5531686547. PC-61b (first-char swallow) recorded as likely same family.

**Deliverable 2 — #3137 still reproduces at HEAD 64b75a64d** (Move-to-Folder popup: ↓ dismissed the popup + moved the dock selection, 1/1). Recorded only — the owner's own master repro stands; no comment (frugal rule).

**Deliverable 3 — retained-mode keyboard-ownership sweep (Run #62 (a)) PASS across the other surfaces:** New Task… ▾ dropdown (↓↑ move within `create` popup, Esc closes), Alt+P project dropdown (opens + auto-expands Filters fold; ↓ moves within; Esc), Manage modal workspace-list arrows (heavy-frame selection moves), tab context menu (arrows navigate; printable `q` neither leaks to the terminal behind nor closes the menu; Esc), tab-bar '+' popup (New Terminal/New File; same grab checks). Only the Move-to-Folder popup (#3137) remains broken.

**Deliverable 4 — new coverage:** NEW welcome screen smoke PASS (opens once as a tab; ladder key `3` → LEVEL 3 orchestrator card with live workspace glyphs + action buttons). `fresh --skill` prints the scripting guide. `fresh --cmd script run` from INSIDE an embedded terminal drives the running editor (returned `["terminal"]` for the workspace's pane list). A5.2/A5.3 F9 capture PASS both directions (capture ON: C-p → shell; OFF: palette). A3.3 scrollback keys PASS in scrollback mode (C-Space); live-mode PageUp/C-Home forward to the PTY per docs.

**NEAR-FALSE-POSITIVE (harness):** the '+' button "was completely dead" for 5 clicks — my click columns were computed with `awk index()` = BYTE offsets, but tab bars contain multi-byte width-1 glyphs (× — … ▾ │) so the SGR clicks overshot and missed. Char-indexed recompute (python) → popup opens fine. Rule added to learning_db; audit any future "button doesn't respond to clicks" finding against this first.

**Filed/commented:** 2 comments on #3136 (root cause + scope). NO new issue. Cleanup: killed `fresh-r62-a` tmux session, removed /tmp/rt62. Next run: see Run #63 NEXT in test_plan (watch #3136/#3137 for fixes; Settings deep keys; B9 crash resilience; welcome ladder + sidebar-sections deep-dives).
...
**Objective:** three 0.4.2 changelog claims never black-box-tested:
- (a) **JSONC config (#2497)** — comments + trailing commas tolerated; lenient-slip recovery; no silent reset; save never clobbers an unparseable file (surfaces error instead).
- (b) **Persistable/promotable macros (#2487)** — Macro: Save to init.ts (re-seeds register at startup) + Macro: Promote to command.
- (c) **Per-buffer view toggles** — Toggle Line Numbers / Line Wrap (Current Buffer): active-buffer scope + persistence across restart.

**Method:** single tmux session `fresh-r53` (120x40), release binary, project + user + `--config` config layers, real fixtures in `/tmp/{jsonc53,macro53,view53}`.

### (a) JSONC config #2497 — COMPREHENSIVE PASS (fix confirmed; save is comment-PRESERVING)
- **Project `.fresh/config.json` with `// line`, `/* block */` comments + trailing commas** → editor launches clean (no `[⚠]`), Settings→Tab Size shows **7** (the commented value), not the default 4. Comment no longer discards the file.
- **`--config file.json` with comments** → editor **starts** and applies settings (Tab pressed → 2-space indent from `tab_size:2`); the old "Parse error: key must be a string" hard-exit is gone.
- **Lenient-slip recovery** — a config with a missing comma between `tab_size` and `line_numbers` AND an unquoted `editor:` key → still loads (Tab inserted 9 spaces from `tab_size:9`), no reset to defaults, no warning badge. Matches docs' "small slips tolerated".
- **Genuinely broken file** (unterminated object, missing `}`) → loads DEFAULTS + surfaces a real warning: `[⚠ 1]` badge; "Show Warnings" log reads `Failed to load layered config: Parse error: <path>: Parse error: Unterminated object on line 4 column 1, using defaults`. Not silent.
- **No-clobber on save (#2497 headline)** — with a broken USER `~/.config/fresh/config.json`, edited Tab Size in Settings UI + `Ctrl+S` → modal **"Couldn't save settings"** dialog: "Failed to save settings: Parse error: … Unterminated object on line 4 column 1 / Your config file was left unchanged so you can fix it. / Press Esc to open your config file and fix it." **File md5 byte-identical before and after the failed save** (verified) — never clobbered. **Esc → opens the broken config.json in-editor** for fixing (status "Editing User config: …", filetype JSON).
- **Positive save is COMMENT-PRESERVING** — fixed the user config to valid JSONC (`// comment` + trailing comma, `tab_size:6`), relaunch loads it (Settings shows 6, no badge); then Settings→set Tab Size=2→`Ctrl+S` → "Settings saved to User layer"; on-disk file still has the `// user-level config with a comment` line and the `/* trailing comma follows */` intact — save did a **surgical value edit** (`6`→`2`), not a strip-and-rewrite. Better than the changelog even claims.

### (b) Macros save/promote #2487 — COMPREHENSIVE PASS
- **Record** — palette "Record Macro" → "Record macro (0-9):" prompt → `0`+Enter → status "Recording macro '0' (F5 or Ctrl+P → Stop Recording)". Recorded End/`-DONE`/Home/Down; **F5** → "Macro '0' saved (8 actions)". **F4** replays correctly on the next line (`beta` → `beta-DONE`, cursor advances).
- **Macro: Save to init.ts** → "Save macro to init.ts (0-9):" → `0` → "Macro '0' saved to init.ts (reloaded)". Generated `~/.config/fresh/init.ts` (full starter template + a sentinel block):
  ```
  // fresh:macro 0 — generated by Fresh, editable
  // types: "-DONE"
  editor.defineMacro("0", [ {action:"move_line_end"}, {action:"insert_char",args:{char:"-"}}, …×5, {action:"smart_home"}, {action:"move_down"} ]);
  // fresh:end macro 0
  ```
  Exactly the 8 ActionSpecs, plus the `// types:` summary comment.
- **Re-seed across restart** — quit, relaunch fresh (init.ts loads clean, no error), palette "Play Macro" → `0` → macro plays from the register that init.ts seeded at startup (`one` → `one-DONE`). Persistence claim verified.
- **Macro: Promote to command** → "Promote macro to command (0-9):" → `0` → "Macro '0' promoted to an editable command in init.ts". The **sentinel block was UPSERTED in place** (the old `defineMacro` block replaced by):
  ```
  // fresh:macro 0 — promoted to a command; edit freely
  // types: "-DONE"
  registerHandler("macro_0", async function () { await editor.executeActions([ …8 steps… ]); /* arbitrary logic */ });
  editor.registerCommand("Macro 0", "Run promoted macro 0", "macro_0");
  // fresh:end macro 0
  ```
- **Promoted command runs** — palette shows "Macro 0" (source tag `init.ts`, desc "Run promoted macro 0"); running it on line 2 → `two` → `two-DONE`. End-to-end record→save→reseed→promote→run all work.

### (c) Per-buffer view toggles — COMPREHENSIVE PASS (+ important restore nuance)
- Both commands exist in TWO variants each: global "Toggle Line Numbers"/"Toggle Line Wrap" AND "… (Current Buffer)".
- **Scoping** — opened long.txt (300-char line) + other.txt. "Toggle Line Numbers (Current Buffer)" on long.txt → "Line numbers hidden", gutter gone on long.txt; switched to other.txt → **gutter (1,2,3) still present**. Active-buffer scope confirmed. "Toggle Line Wrap (Current Buffer)" on long.txt → "Line wrap disabled", the 300-char line stops wrapping (single row, `last line` now visible).
- **Persistence across restart — WORKS, but only via pure session restore.** Toggled both off, quit, relaunched **with no file args** (`fresh`, restore-enabled) → long.txt reopened with line numbers **still hidden** AND wrap **still disabled**. Verified both independently.
- **NUANCE (not a bug):** relaunching with an EXPLICIT file arg (`fresh long.txt`) opens a **fresh buffer with defaults** — it does NOT restore that buffer's saved per-buffer view state (gutter + wrap both came back). Session-restored view state only applies on the pure-restore path (no file argument). Standard editor behavior (VS Code distinguishes "open file" from "restore workspace"). Recorded as a harness lesson so future runs don't misread it as a persistence bug.

**Result:** all three targets PASS, **no bug, no R3 nit worth batching.** Notable positives beyond the changelog: JSONC save is comment-preserving (surgical), and the no-clobber path even offers Esc-to-open-and-fix.

**Cleanup:** killed `fresh-r53`; removed `/tmp/{jsonc53,macro53,view53}` and the run-created `~/.config/fresh/{config.json,init.ts,tsconfig.json,types}` (dir was empty at run start — restored to that state; only `logs/` remains).

See learning_db.md additions: "JSONC config + macros save/promote + per-buffer view toggles (Run #53)".
...
## Run #46 — 2026-07-06 — 5 fixed-bug rechecks (4 confirmed FIXED, #2312 still broken) + Virtual Space new coverage → BUG #2577 — v0.4.3 @ 9f6135001

**Preflight:** Synced state branch (clean). Master FORCE-UPDATED past Run #45's `1c6bd8ce9` → **`9f6135001`**, now **v0.4.3** (two releases landed since Run #45: 0.4.2 + 0.4.3). Read CHANGELOG 0.4.2/0.4.3 first. Per **R1** (version changed + multiple agent-issue fixes landed): primary objective = recheck the closed agent issues. Per **R2**: advanced NEW coverage = **Virtual space** (0.4.3 headline). Playbook intact; lessons continuity OK; GitHub MCP auth live (`list_issues`/`search_issues`/`issue_read`/`add_issue_comment`/`issue_write` all returned). 14 OPEN `tui-agent-auto-bug` issues (all vi-mode + older behavioral); the 5 issues rechecked this run were already CLOSED by the maintainer in 0.4.2/0.4.3.

**Build:** built release `fresh` 0.4.3 from a fresh origin/master worktree `/tmp/fresh-master` @ `9f6135001` (container reclaimed; from-scratch build, exit 0, 32 MB binary). Removed worktree at cleanup.

**FIXED-BUG RECHECKS (black-box UI, all verified against the current binary):**
- **#2309 (RO indicator) — CONFIRMED FIXED.** `fresh --no-restore /usr/include/stdio.h` → status bar now renders a persistent **`[RO]`** segment (between the filetype and `LF`). Persists across cursor movement (Ln 3 after Down×2, still `[RO]`). **Toggle Read-Only Mode** removes `[RO]` (status `Read-only mode disabled`) and re-adds it (`Read-only mode enabled`). Matches CHANGELOG 0.4.2 "Read-only `[RO]` indicator now actually renders … and is clickable (#2309)".
- **#2314 (configurable indent rules) — CONFIRMED FIXED (core).** Project `.fresh/config.json` custom language `inc46lang` (ext `inc46`) with `increase_indent_pattern:"OPEN\s*$"`. **Control** (line ends "no token") → next line `child` at col 0. **Positive** (line ends `OPEN`) → next line `child` indented **4 spaces**. In Run #34 this exact non-built-in-token test was a pure no-op; the custom pattern now fires → CHANGELOG 0.4.2 "Per-language indentation rules are now actually applied (#2314)". **Nuance (NOT filed, murky → test_plan pending):** `decrease_indent_pattern:"^\s*CLOSE\b"` did NOT self-dedent the matching `CLOSE` line (stayed at indent 4 where VS Code / Fresh's own doc say it should dedent to 0), though it appeared to affect the *following* line's baseline in one capture. Inconsistent across captures → per filing standards did not file from a murky state; flagged for a focused characterization next run.
- **#2315 (untracked directories in Review Diff) — CONFIRMED FIXED.** Real git repo with an untracked `assets/` dir (2 files). Review Diff now **expands** it: sidebar `UNTRACKED (2)` → `▾ assets/ +2 -0` with named rows `? icon.txt +1 -0` and `? logo.txt +1 -0`; center lists `assets/icon.txt +1/-0` and `assets/logo.txt +1/-0` individually with content. In Run #35 this was a single blank-named `(untracked directory)` placeholder. Matches CHANGELOG 0.4.2 "#2373"/untracked expansion work.
- **#2420 (i18n discard key leak) — CONFIRMED FIXED.** Line-level discard (`v` on `-` line, `j` to extend over the `+`, `d`) on a single-line modification → status bar now shows the localized **`Lines discarded`** (git-verified the modification reverted, file clean). In Run #38 this printed the raw key `status.lines_discardd` (typo). Matches CHANGELOG 0.4.2 "discard shows a localized message (#2420)".
- **#2312 (occurrence highlight ignores theme) — STILL REPRODUCES in v0.4.3 (issue was closed as completed).** Re-verified black-box with ON/OFF differential ANSI capture, no config, `items` repeated on 4 lines. **light theme:** occurrence-highlight ON draws all 4 `items` with `\e[48;5;16m` (near-black box on a light bg); OFF → 0 boxes (`48;5;16mitems` count = 4 ON / 0 OFF → the boxes ARE the highlight and its color is fixed palette index 16). **high-contrast:** ON/OFF differential unchanged on non-current lines (highlight bg 16 == editor bg 16 → invisible); only the word under the cursor drawn bg 16 on current-line bg 233 (recessed). Color still hard-wired to index 16, not theme-derived. → **Commented on the closed #2312** with the v0.4.3 evidence (flagging premature close); did NOT re-file a duplicate.

**NEW COVERAGE (R2) — Virtual space (0.4.3 headline, `editor.virtual_space`) — COMPREHENSIVE PASS + 1 BUG FILED (#2577).** Palette **"Toggle Virtual Space (Current Buffer)"** (builtin, no default key, "Turn virtual space on or off for the current buffer only"). Fixture with varied line lengths (`--no-restore`).
- **Default OFF (baseline):** on `short` (5 chars), End→Col 6, Right at EOL **wraps** to next line (Ln 3, Col 1). Correct.
- **Toggle → "Virtual space: on (this buffer)".**
- **Horizontal past EOL — WORKS:** End on `short` then Right no longer wraps; cursor advances into the void (verified: after 6 Rights, typing `X` produced `short      X` = 6 spaces + X = column 12; and via `tmux display-message '#{cursor_x}'` the on-screen cursor advanced screen col 11→14 for 3 Rights = text col 9). Materialization fills the gap with spaces (CHANGELOG match).
- **Vertical below last line — WORKS:** ArrowDown past the last line parks the cursor on virtual lines (screen cursor row advances); typing `VOID` two lines down materialized empty lines 6-8 (newlines) + `VOID` at line 9 col 11 (10 leading spaces) — "typing anywhere in the void fills the gap with newlines and spaces" ✓.
- **BUG → #2577 (low–med, FILED): status-bar Ln/Col indicator FREEZES at the line-end position while the cursor is in virtual space.** During horizontal virtual-space movement the `Col` readout stays frozen at the EOL column (`Col 6`) across any number of Right presses while the real cursor advances (proven by `#{cursor_x}` = text col 9 and by the `X` materialization at col 12); during vertical virtual space BOTH `Ln` and `Col` freeze at the last real position. Readout only corrects when the cursor lands on real text again. Reference = Visual Studio (the feature's stated model), which reports the virtual column. Distinct from #2301 (post-jump Ln staleness) in trigger + symptom; noted the possible shared status-bar-refresh path. Reproduced from a clean `--no-restore` launch. 3 dup-search variations, 0 hits.

**HARNESS notes (learning_db):** (1) `[RO]` now a real status segment — grep the status line for `[RO]`. (2) Virtual-space cursor position is NOT readable from the status bar (frozen, per #2577) — verify true caret position via `tmux display-message -p '#{cursor_x}'`/`#{cursor_y}` OR by typing a marker and reading where it materializes. (3) Review Diff center pane expands ONLY the FOCUSED file's hunk; other files render header-only — navigate focus with `,`/`.` (the `▸GROUP · file` breadcrumb shows the focused file) before line-navigating, else the hunk is invisible. (4) The line-level discard SUCCESS message ("Lines discarded") is briefly overwritten by the "Review Diff: Hunk 1 of 1" refresh status — capture at ~0.15s AND ~0.4s after `d`. (5) `--no-restore` keeps the on-disk file pristine even after heavy virtual-space edits (never saved) — safe to relaunch for a clean repro. (6) File-mode palette fuzzy search is scoped to the workspace root, not `/tmp` — relaunch pointed at the fixture instead. (7) Occurrence-highlight color proof: count `48;5;16mitems` in ON vs OFF `-e` captures.

**Notification:** sent (1 new confirmed bug #2577 in the flagship 0.4.3 feature + a wrongly-closed bug #2312 still reproducing — both a maintainer actively on virtual space / theming would act on).

**State updates:** run_log (this entry), learning_db (+"Virtual space (Run #46)" + "[RO] indicator FIXED / occurrence-highlight #2312 still broken (Run #46)"), confirmed_bugs (+BUG-022/#2577), github_issues (+#2577 row; marked #2309/#2314/#2315/#2420 FIXED, #2312 still-reproduces note; Last-updated bump), test_plan (Run #46 note + Run #47 NEXT + decrease_indent pending).

**Cleanup:** killed tmux `qa46`; removed `/tmp/vs46`, `/tmp/indent46`, `/tmp/rdiff46`, `/tmp/occ46.py`, `/tmp/occ_*.ansi`; removed build worktree `/tmp/fresh-master`.

**NEXT new-coverage (Run #47+, top-down, prefer freshest 0.4.3):** (a) finish Virtual space: `block` mode (`editor.virtual_space:"block"` → only block selections extend past line ends; true-rectangle copy/insert), per-buffer persistence across restart (NOT `--no-restore`), click-past-EOL, block selection into virtual space; (b) characterize `decrease_indent_pattern` self-dedent precisely (murky this run — is it a real deviation from VS Code/its own doc, or does it dedent the following line by design?); (c) **Slang shader** support + Go-to-Definition into read-only builtin modules (0.4.3, #2536/#2539 — needs slangd; may not be installed); (d) 0.4.2 "Step through Search & Replace matches from the editor" (`Ctrl+Alt+→/←`); (e) 0.4.2 indentation guides (off/all/active-block, custom glyph); (f) '+' new-tab popup / terminal Ctrl+Click / OSC 7. Then #2197 pyright only if a fix lands.

---

## Run #47 — 2026-07-07 — Virtual space block/persistence/click ALL PASS + decrease_indent characterization → BUG #2582; SGR mouse harness breakthrough — v0.4.3 @ 9f6135001

**Preflight:** Synced state branch (clean). Master UNCHANGED at **`9f6135001`** (v0.4.3) since Run #46 → per **R1** skipped fixed-bug rechecks (no new fix landed; 14 open `tui-agent-auto-bug` issues). Playbook intact (all sections). Lessons continuity OK (learning_db topics through Run #46 present). GitHub MCP auth live (`list_issues` returned the open set).

**Build:** release `fresh` 0.4.3 from a fresh origin/master worktree `/tmp/fresh-master` @ `9f6135001` (container reclaimed; from-scratch build 6m35s, exit 0).

**(a) Virtual space completion (Run #46 NEXT) — COMPREHENSIVE PASS, no new bug:**
- **`block` mode** (`/tmp/vs47block/.fresh/config.json` → `{"editor":{"virtual_space":"block"}}`, ragged 5-line fixture): plain Right at EOL of `short` WRAPS (Ln3 Col1 — arrows stay non-virtual ✓); **block selection M-S-Down×3 + M-S-Right×6 renders a true rectangle** — highlight `48;5;17` continues through virtual cells on short lines (2-char `ab` row: 7 highlighted virtual cells); **OFF-mode control clamps** (same selection over `short`: zero virtual highlight, block cursor clamps to EOL) proving the config switches behavior; **rectangular copy**: paste of the copied block yields 4 fragments EXACTLY rect-width — ragged rows space-padded (`rt    `), fully-virtual row = all spaces — disk-verified (`Ctrl+S` + `od -c`); **single-cursor paste** = VS Code column-paste (fragment 1 at cursor, rest as new lines; #1057 parity, NOT a bug); **typing into a block selection spanning virtual cells** pads + inserts at the true column on every line (`medium Xlength…` / `ab     X`) ✓ "rectangular copy/insert operate on true rectangles" CHANGELOG claim fully verified.
- **Toggle cycle from a block-config buffer: block → on → off → on…** — "Toggle Virtual Space (Current Buffer)" switches off↔on only; no path back to block per-buffer → **IMP-024** (low, not filed per R3).
- **Per-buffer persistence across restart** (`/tmp/vs47`, NO `--no-restore`): toggle on → quit → relaunch: buffer restores with virtual space STILL ON and the cursor's VIRTUAL position itself restored (screen col proves col 8 on the 5-char line; typed marker landed `short    P` = col 10 after 2 more Rights). PASS.
- **Click-past-EOL + click-below-last-line** (SGR mouse, see harness below): click at text col 40 on the 23-char line 3 → caret parks at col 40 (`cursor_x`-verified; `Q` materialized at col 40 after 16 pad spaces); click 4 rows below the last line → caret parks there; typing `V` filled newlines + 13 pad spaces exactly like keyboard vertical virtual space. PASS. Status-bar Ln/Col freeze also triggers on click = existing **#2577** (not re-filed).

**(b) `decrease_indent_pattern` self-dedent characterization (Run #46 murk) — RESOLVED → BUG FILED #2582 (med):**
- Custom lang `dec47lang` (`/tmp/indent47/.fresh/config.json`, increase `OPEN\s*$`, decrease `^\s*CLOSE\b`, dedent-next `^\s*RET\b`):
  - `increase` fires (child line → 4sp) ✓ (#2314 fix holds).
  - **Enter-split WORKS per docs**: cursor before `CLOSE tail` on an indent-4 line, Enter → moved-down line lands at col 0 (one level shallower) ✓ — this is what Run #46 glimpsed as "affects the following line".
  - **Typing the trigger NEVER re-indents**: `CLOSE` typed on an inherited-indent-4 line stays at 4 (2 trials); Enter at END of a `    CLOSE two` line → next line inherits 4 (no retro correction).
  - `dedent_next_line_pattern` WORKS (`    RET x` → next line col 0) — closes the last unverified row of #2314's table except one:
- **Built-in comparison:** Python `if a:`/`x = 1`/then type `else:` → stays `    else:` (NO live dedent; VS Code/Sublime/Vim dedent live) and the following line compounds to 8. C control: type `}` on an indent-4 line → **live-dedents to col 1** (electric bracket). So live re-indent exists ONLY for closing brackets; keyword triggers (built-in AND custom) never get it. This exact "type CLOSE on an indented line → dedents" case was a row in closed #2314's expected table → filed focused follow-up **#2582** (4 dup-search variations 0 hits: `else dedent`, `indent typing keyword`, `auto-indent else elif`, `decrease_indent_pattern`). → confirmed_bugs BUG-023, github_issues row + Last-updated bump.

**HARNESS BREAKTHROUGH (learning_db):** mouse IS drivable through tmux — previous runs' "tmux mouse passthrough" blocker was wrong for clicks. Raw SGR via `send-keys -l`: press `\e[<0;COL;ROWM` + release `…m` (1-based; buffer line N = row N+2, text col C = col C+6 on 220×50). Verified end-to-end twice (horizontal + vertical click). Unblocks right-click menus / Ctrl+Click / drag / scroll-wheel coverage (Run #48 candidate (c)). Other gotchas: `M-g` Go-to-line prompt opens slowly and swallows early digits — confirm the digit rendered before Enter (or arrow-navigate small files); `C-v` while a prompt is open pastes INTO the prompt.

**State updates:** run_log (this entry), learning_db (+2 topics), confirmed_bugs (+BUG-023/#2582), github_issues (+#2582 row + Last-updated), potential_improvements (+IMP-024), test_plan (Run #47 note + RUN #48 priority order).
...
## Run #50 — 2026-07-07 — #2442 CONFIRMED FIXED; shader grammars ALL PASS; File Explorer context menu first test → BUG #2587 — v0.4.3 @ 4e945b494

**Preflight:** Synced state branch (clean). Master FORCE-UPDATED past Run #49's `89d91e84d` → **`4e945b494`** (still v0.4.3; 10 new commits: the monorepo/multi-repo nested-git-workspace series `3a78ad5ec`…`0bfb54edf` incl. relative find-file paths + blame-history fix, two watch_path e2e deflakes, and **`4e945b494` "fix(vi): clamp j/k onto a shorter line to its last character (#2442)"** — a fix for an OPEN agent issue → recheck mandatory). Playbook intact (all sections). Lessons continuity OK (topic-reorg note stands). GitHub MCP auth live (list_issues returned the open set). Built release binary from scratchpad worktree @ 4e945b494 (background build, exit 0).

**(1) Fixed-bug recheck — #2442 CONFIRMED FIXED (per preflight step 4):** all 3 original failure paths re-run black-box in vi mode on the original fixture: `$` on the long line → `j` onto `short` lands **Col 5** (was 6) and `x` → `shor` with line 3 intact (no join); col-10 `j` clamps to 5 AND continuing `j` restores the goal column (**Col 10**, Vim desired-column — a bonus beyond the reported clamp); `k` from below → Col 5, `x` → `shor`. Commented "confirmed fixed in v0.4.3 @ 4e945b494" on #2442; marked FIXED in github_issues + confirmed_bugs (BUG-020).

**(2) RUN #50 priority (a) — GLSL/HLSL/WGSL grammar smoke (commit `89d91e84d`) + Slang: ALL PASS, no bug.** `.glsl`/`.hlsl`/`.wgsl`/`.slang` fixtures each get the right status-bar filetype (`GLSL`/`HLSL`/`WGSL`/`Slang`) and REAL syntax coloring (ANSI-verified: keywords `38;5;51`, comments `253`, numbers `69`, fn names `226`, strings `34`, rainbow brackets; WGSL `@attributes` + `var<uniform>` generics and Slang `[shader("fragment")]` + semantics all colored). Slang shows an **`LSP (off)`** segment (slangd registered but absent in this container) → 0.4.3 Slang Go-to-Def into builtin modules (#2536/#2539) stays UNTESTABLE here; the other three have no LSP segment.

**(3) RUN #50 priority (b) — File Explorer right-click context menu (first-ever test, SGR mouse harness): menu comprehensive PASS on function, 1 BUG FILED → #2587 (med).**
- Menu on file/dir/empty-space right-click: New File / New Directory / Rename / Cut / Copy / Paste / Delete / Duplicate / Copy Full Path / Copy Relative Path. Verified end-to-end: Duplicate (`alpha copy.txt`, disk-verified), Delete (**y+Enter typed prompt**, "Moved to trash"), New Directory (prefilled `New Folder <ts>`, C-u clears), Rename (prefilled current name, disk-verified), New File (created in the selected dir), Copy Full/Relative Path (clipboard-verified via paste). Arrows/Enter/Esc go to the menu; right-click sets the selection to the clicked row.
- **BUG #2587:** the menu does NOT grab printable keys/Backspace — they fall through to the explorer **type-ahead find** (header → `/x`), which **moves the selection behind the open menu**, and menu actions follow the live selection: right-click `alpha.txt` → type `sub` → Copy Relative Path copied **`subdir`**. 3/3 leaks (`q`/`z`/`sub`); leaked filter persists after the menu closes. Contrast: tab context menu + '+' popup grab the keyboard (0.4.2 changelog, re-verified Run #49). Dup-searched 3 ways, 0 hits (#1576 = the closed feature request; #2244 = hover suppression). → BUG-025, github_issues row.
- Papercuts → **IMP-026** (R3, not filed): empty-space right-click shows the full selection-dependent menu (VS Code reduces it); New File status leaks internal `Renamed untitled_<ts>.txt to <name>`; timestamped New-Folder default.

**HARNESS (learning_db):** palette-open race — confirm the `>` prompt row exists before typing the query (a 0.6s-early query typed into the buffer and cost a discard+relaunch); explorer tree rows shift after expand/create — recount SGR rows from a fresh capture before each click; explorer type-ahead = jump-to-match (`/str` header, tree NOT narrowed), Esc clears.

**State updates:** run_log (this entry), learning_db (+3 topics), confirmed_bugs (BUG-020 fixed, +BUG-025/#2587), github_issues (+#2587 row, #2442 → FIXED, Last-updated bump), potential_improvements (+IMP-026), test_plan (Run #50 note + RUN #51 priority order).
...
## Run #56 — 2026-07-07 — #2437+#2439 CONFIRMED FIXED; LSP diagnostics deep-dive + signature help + code actions + auto-import → 4 BUGS FILED #2601–#2604 — v0.4.3 @ 11dccfad5

**Preflight:** state synced; playbook intact; auth live (get_me OK). Master FORCE-UPDATED `4e945b494` → **`11dccfad5`** (still v0.4.3; exactly 2 commits, both vi fixes for OPEN agent issues): `463c435e3` fixes #2437 (`cw`), `11dccfad5` fixes #2439 (`i"`/`a"` forward search). Both auto-closed as completed. slangd still ABSENT → Slang priority stays blocked. 22 open agent issues, ZERO maintainer comments on the Run #48–55 filings. Built release from `/tmp/fresh-master` worktree (~7min cold).

**Fixed-bug rechecks (preflight step 4) — BOTH CONFIRMED FIXED via UI, commented on both:**
- **#2439:** `di"` from col 1 of `the "quick" brown fox` → `the "" brown fox`; `ci"` from col 1 → INSERT + `the "WXYZ" brown fox`; `da"` forward-searches too. Marked resolved in registry.
- **#2437:** `cw`+`X` at col 1 of `hello world foo bar` → `X world foo bar` (space preserved = `ce`/Vim); mid-word `heX world foo bar`; control `dw` still eats the space. Marked resolved.
- **NEW BUG found during the recheck → #2604 (med-low):** `a"` excludes the trailing whitespace, unlike Vim `:help aquote` — `da"` yields `the  brown fox` (double space, `cat -A`-verified) from col 1 AND from inside the quotes (2/2). `di"` unaffected.

**Objective (R2):** Run #56 priorities (a) LSP diagnostics deep-dive, (b) signature help, (c) Alt+. code actions, (d) auto-import on completion accept — all via rust-analyzer 1.94.1 Full mode on fixture crate `/tmp/lsp56` (struct Point + describe(p,verbose) + call sites; `rustup component add rust-analyzer` from crate dir; tmux `qa56` 220x50).

### (a) Diagnostics deep-dive — core PASS + 2 BUGS
**PASS:** RA-native missing-fields publishes LIVE (no save, ~2-3s, red ● + E:N); save → flycheck errors AND warnings AND hints (status segments `E:1 W:2 I:1`; hints labeled I in status, [H] in panel); **hover fusion** (Alt+K): `✖ Error (rustc)` bold `38;5;203` / `⚠ Warning` `38;5;226`, source-tagged, message + related info in popup; **F8/Shift+F8** wrap-around incl. TWO same-position diagnostics cycled distinctly (message echoed in status); **panel**: cursor-driven RO buffer (`Diagnostics: N items | a: toggle filter | RET: goto | q: close`), Enter on [E] row → `Jumped to <abs>:23`, exact col 23:13, editor focused, panel closes; `q`/Escape close; `a` toggle WORKS (adds other-file sections); Up/Down PREVIEW scrolls editor and even opens external files (as real tabs); **inline diagnostics** `diagnostics_inline_text` via Settings UI /-search → right-aligned EOL text, severity-coloured (203/226), highest-severity per line, live-applies; **clears-on-fix LIVE** (add missing-fields line → E+1 in ~3s; undo → clears without save).
- **BUG #2601 (med):** dismissing a hover popup (Escape OR cursor-move) whose position does NOT overlap an error removes that error from gutter+status count+F8 until next save; panel still lists it. 5/5. Controls: hover ON the error preserves (2/2); bare Esc, panel open/close preserve.
- **BUG #2602 (med):** diagnostic positions frozen at publish time — after "Generate `new`" inserted 6 lines above the error, ● stayed on old line 8 (inside the GENERATED fn), F8 jumped to 8:17 mid-generated-code, real error line unmarked; corrects on save. Same family as #2583 (S&R) but diagnostics subsystem.
- Pending (NOT filed): new `src/bin/two.rs` created while RA running never got diagnostics even after save — target discovery? characterize before filing.

### (b) Signature help — COMPREHENSIVE PASS
Auto-triggers on `(` (no palette cmd needed). Popup `Signature Help [Alt+T to focus]`: signature line, blank, ACTIVE PARAMETER on its own line, separator rule, markdown doc paragraph (`String::from(` → "Converts to this type from the input type."). Active param line ADVANCES on comma (`p: &Point` → `verbose: bool`). Esc dismisses. (Earlier "no active-param highlight" observation was a truncated capture — false positive avoided; presentation = dedicated line, not inline bold.) Hanging-indent claim untested (needs long wrapped signature).

### (c) Alt+. code actions — PASS (both families)
- Diagnostic quick-fix: Alt+. on missing-fields error → `1. Fill struct fields quickfix` → press `1` → `Point { x: 3, y: todo!(), label: todo!() }`, `Applied: Fill struct fields (2 change(s))`, E count drops. (The #2212 clangd gap does NOT affect rust-analyzer.)
- Non-diagnostic assists: Alt+. on struct name → 4 assists (Convert to tuple struct / Generate impl / Generate trait impl / Generate `new`); arrow highlight `48;5;25` + Enter → full impl block inserted, `Applied: Generate `new` (1 change(s))`. WorkspaceEdit assists end-to-end = CHANGELOG #1915 claim verified.

### (d) Auto-import on completion accept — BUG #2603 (med), docs claim unreachable
`HashMa`/`HashMap` bare ident + Ctrl+Space (and palette Show Completions) → NO popup at all, ever. Trait-method needing import (`p.type_i`) → nothing. CONTROLS same session: `p.` members ✓, `Strin` → String+stringify! (prelude) ✓, `std::collections::HashMa` → HashMap (qualified) ✓. So every candidate requiring an import is missing → "Auto-imports are applied when you accept a completion" (lsp.md) can never trigger.

**Nits → potential_improvements IMP-029 batch (R3):** panel header stays "(Current File)" after `a`-toggle (status msg + list change prove the filter switched); panel item count ignores the filter; status-bar echoes concatenate multi-messages/multiline without separator ("of `Point`missing `label`", "fields:- y- label"); gutter ● always red `38;5;160` even for warning-only lines (hover/inline are severity-coloured); panel truncates RA multiline message to first line; related-info rendered as bare top-level "mod.rs:" section (VS Code nests under parent); panel preview opens external files as persistent tabs.

**Harness lessons (learning_db):** tmux `send-keys` swallows a TRAILING unescaped `;` — both as a bare arg AND at the end of a `-l` string (tmux command separator). Send `'\;'`. Cost 3 rounds of "dead `;` key" investigation — false positive avoided. Palette-open race again (typed into buffer; recovered per-char C-z). BSpace bursts overshoot (ate preceding lines); undo bursts roll back WorkspaceEdits too — prefer palette "Revert File" (prompt: `r`+Enter) for fixture reset. Completion absent ≠ broken — verify with `p.`/prelude controls before concluding.

**State updates:** run_log (this), learning_db (+1 topic), confirmed_bugs (+BUG-031..034), github_issues (+4 rows, #2437/#2439 → Closed-verified, header bump), potential_improvements (+IMP-029), test_plan (Run #56 note + RUN #57 order).
...
## Run #59c — 2026-07-20 — v0.4.4 @ f545a75ad — Embedded terminals + workspace orchestration, basics→fancy (user directive) → BUG #2735 (extracted co-tenant workspace not persisted); folders #2703 + terminal mouse selection COMPREHENSIVE PASS

**Preflight:** state synced; playbook + lessons intact; auth live. **Master ADVANCED 0.4.3→0.4.4** (@ f545a75ad; prior Run #58 hash 6ab255709 no longer in history — master was rebased/replaced). Rebuilt release from scratch (container reclaimed; 8m41s, exit 0). Open agent issues dropped **25→12** (maintainer closed ~13 in 0.4.4). Cross-checked all 12 still-open agent issues against the 0.4.4 changelog fix list: **none are claimed fixed in 0.4.4** (the closed ones already moved off the open list), so no fixed-bug recheck was indicated this run — deferred the LSP/vi/deferred open issues (all outside the emphasis area) to a future run.

**User mid-run directive:** start from the BASICS of orchestration and/or embedded terminals before the newfangled features; build a comprehensive plan covering every user flow basic→fancy. Emphasis areas: workspace orchestration + embedded terminals. Ignore: vi mode. → Wrote a comprehensive plan (AREA A terminals A1–A11, AREA B orchestration B1–B9), annotated prior coverage, executed the uncovered items bottom-up.

## EMBEDDED TERMINALS (comprehensive)
- **Open (A1):** "Open Terminal" (current split, tab), "Open Terminal to the Right" (vsplit), "Open Terminal Below" (hsplit) — all PASS. Terminal tabs auto-disambiguate duplicates in same cwd as `projA 1`, `projA 2`.
- **Interaction (A2):** echo/arith (`MARKER-A2-7`), `pwd`=workspace root, sequential cmds, interactive stdin (`read`→GOT=Alice), long output scroll — all PASS.
- **Modes (A3):** Ctrl+Space live↔read-only PASS; Ctrl+Home/Ctrl+End jump top/bottom of scrollback PASS; Ctrl+F scrollback search highlights match + "Found N match" + moves cursor PASS. **Ctrl+] (documented "exit terminal mode, same as Ctrl+Space") is a NO-OP** in plain tmux — the 0x1D byte is forwarded to the shell, mode unchanged; Ctrl+Space works. Likely terminal-encoding (Ctrl+]=0x1D, like Ctrl+H #2109) → logged as pending candidate / IMP, NOT filed.
- **PTY sizing (A6.2):** focused split reports `tput cols`x`lines`=108x21 matching split geometry. PASS.
- **Mouse text selection (A10, NEW 0.4.4) — COMPREHENSIVE PASS via SGR harness:** drag on live terminal → drops to read-only scrollback + real selection (bg 48;5;17); Ctrl+C → "Copied - terminal resumed" and clipboard paste = exact selected span; double-click selects word (stops at `-` delim), triple-click selects whole line; bare click focuses only (stays live, no selection); mouse forwarding — a shell that enabled DECSET 1000/1006 (`cat -v`) RECEIVES plain-drag SGR bytes (grid-local coords), while **Shift+drag (button+4) bypasses forwarding and selects** (drops to read-only, no bytes forwarded) — full xterm convention.
- **NextPane (A11.1, #2562):** "Next Pane" cycles split+tab; landing on the terminal enters terminal mode (typed `ZZTOP` reached the shell). PASS.
- **Per-split scrollback (A11.2, #2595) — COMPREHENSIVE PASS:** Split Vertical shows the SAME terminal in two splits; put right split in scrollback@top (Ctrl+Home), typed a live command from the left → left follows live output (`LIVEOUT-NEW` at bottom) while right stays frozen at old history — independent modes AND scroll positions; the scrollback split did NOT auto-jump on new output.
- **Scrollback persistence vs --no-restore (pending candidate):** terminal scrollback persists per-workspace on disk at `~/.local/share/fresh/terminals/<enc-root>/fresh-terminal-N.txt`; launching `fresh --no-restore` and opening a fresh terminal RELOADS the previous session's scrollback (deterministic). `--no-restore` is documented only as "Don't restore the previous **workspace**"; scrollback persistence is a separate documented feature, so surprising-but-plausibly-intended. Logged as pending, NOT filed.

## WORKSPACE ORCHESTRATION
- **Condensed toolbar (B3.2, #2703):** dock now shows `[ New Task… ▾ ] [Search Tasks]` + collapsible `[ ▸ Filters ]`. Confirmed.
- **Workspace folders (B7, #2703) — COMPREHENSIVE PASS:** "Orchestrator: Move to Folder…" → popup `[ Top level ] / [ New Folder… ]`; New Folder dialog (name + "Organize X under this folder" checkbox) creates `▼ ▤ TeamAlpha (1)` with the workspace nested (`┃· projA`). F2 on the folder header → menu **Rename… / New Subfolder… / Delete Folder**. Rename→TeamBeta PASS. Delete Folder removes the folder and returns the workspace to top level (workspace preserved, not deleted). Folder persists across quit+restore (stored in `~/.local/share/fresh/orchestrator/state/orchestrator.json`). Footer hints: `↑↓ switch · →← fold · Enter edit · F2 menu`. Nested subfolders supported.
- **Tab extraction → co-tenant (B8, NEW 0.4.4):** "Extract Tab to New Workspace" moves the current tab into a new co-tenant window over the SAME project root, labeled `projA (2)`. Live: multiple co-tenant windows coexist over one root (verified projA / projA (2) / projA (3)), live-switchable with no restart. Storage is keyed by durable stable_id: `<enc-root>.ws-<stable_id>-<n>.json`.
- **Default filter discovery:** the Orchestrator modal has **"Show empty/1-file workspaces (Alt+I)" OFF by default** — co-tenant windows holding ≤1 file are HIDDEN from the dock AND the "all workspaces" selector until Alt+I. This nearly caused a false "only one co-tenant shows" positive; NOT a bug.
- **BUG #2735 (filed):** **Extract Tab to New Workspace's new co-tenant is not persisted.** Clean repro (2/2): `fresh --no-restore main.rs lib.rs` → extract lib.rs → 2 co-tenant windows (confirmed via modal+Alt+I) → Quit → only ONE workspace file on disk (the SOURCE, main.rs); the extracted lib.rs co-tenant is dropped → restart shows only main.rs (Alt+I confirms no second). Contradicts the 0.4.4 commit "Restore each co-tenant window to its own file on reboot." Both windows are 1-file, so not the Alt+I filter (source survives). Label tui-agent-auto-bug. Searches (0 dupes): `extract tab workspace`, `co-tenant workspace extract restore`, `workspace not restored reboot orchestrator`, `co-tenant window persistence lost`.

**HARNESS notes (Run #59):** (1) Palette file-open flakiness: a stray leading `>` puts the palette in COMMAND mode — typing a filename then matches commands, not files. Always verify the prompt has NO leading `>` before typing a path; `C-u` did not reliably clear it — backspace the whole buffer. Passing files as CLI args (`fresh f1 f2`) is the reliable way to get multiple tabs. (2) Terminal mouse selection IS drivable: plain drag = press `\e[<0;C;RM` + motion `\e[<32;C;RM` + release `\e[<0;C;Rm`; Shift+drag = button+4 (`\e[<4;..M` / motion `\e[<36;..M` / release `\e[<4;..m`); double/triple-click = 2/3 rapid press+release pairs at one coord; terminal grid rows are LOCAL (screen row − terminal-top-offset). (3) Alt+O (`M-o`) reliably toggles dock focus; Alt+I (`M-i`) toggles the 1-file filter inside the modal. (4) Co-tenant state files: `workspaces/<enc-root>.ws-<stable_id>-<n>.json`; orchestrator folder layout: `orchestrator/state/orchestrator.json`; terminal scrollback: `terminals/<enc-root>/fresh-terminal-N.txt`.

**State updates:** run_log (this), test_plan (Run #59 DONE + RUN #60 order + comprehensive terminal/orchestration plan), learning_db (+2 topics), github_issues (+#2735 row, header count 25→12→13-with-2735 open), confirmed_bugs (+BUG-038 #2735 + 2 pending candidates), potential_improvements (+Ctrl+] + scrollback-vs-no-restore notes).

**Cleanup:** killed tmux `fresh-run59`; removed `/tmp/ws59` + `/tmp/cotenant` + fresh workspace/orchestrator/terminal state; kept `/tmp/fresh-master` worktree + target for next run.

---

# Run #60 — 2026-08-24 (v0.4.10 @ 992e61d02) — master v0.4.4→v0.4.10 (BIG jump); Restart-Exited-Terminal (0.4.6 flagship) COMPREHENSIVE PASS → BUG #3074 (tab name not preserved on restart); #2735 CONFIRMED FIXED (both halves)

**Context/harness:** Container reclaimed as usual → recreated `/tmp/fresh-master` worktree from origin/master, full release rebuild (exit 0, v0.4.10). Emphasis (user directive, permanent): workspace orchestration + embedded terminals; ignore vi mode.

**Preflight:** state synced (Run #59c HEAD); playbook + lessons intact; GitHub auth LIVE (`list_issues` OK). **Master version CHANGED v0.4.4→v0.4.10** → preflight step 4 rechecks due. **Open agent-issue count dropped 25→13→4** (maintainer closed a large batch across 0.4.5–0.4.10, incl. our **#2735** fixed 0.4.9). Only 4 open agent issues remain: #2598 (Rust LSP reduced-mode macros), #2318 (Review Diff discard staged), #2197 (pyright timeouts), #2122 (move_to_paragraph keybinding) — all OUTSIDE the emphasis area; LSP rechecks (#2598/#2197) deferred (heavy, off-emphasis). CHANGELOG scan surfaced the emphasis-area targets below.

## (a) Restart an Exited Terminal in place (0.4.6 flagship, docs/features/terminal.md §Restarting an Exited Terminal) — first-ever test, COMPREHENSIVE PASS (covers backlog A9.1)
Fixture `/tmp/wr60` (git repo, main.rs/notes.txt/lib.rs/subdir). ALL documented contracts verified via UI:
- **exit(0) → tab `bash — root@vm: /tmp/wr60 (exited)` + status-bar bottom-left `⟳ Restart terminal`** + "Terminal 0 exited"; buffer drops to read-only scrollback frozen on the last screen (transcript `MARKER-BEFORE-EXIT-60` preserved). ✓
- **exit(3) → `⟳ Restart terminal (exit 3)`** (non-zero code rides along, docs' `(exit N)` claim). ✓
- **Restart via palette "Restart Terminal Process"** → process back **in the same buffer**, tab drops `(exited)`, "Terminal 1 restarted"; **old transcript preserved in scrollback (Ctrl+Home shows `MARKER-BEFORE-EXIT-60`/`exit`) with the new session appended below** (`MARKER-AFTER-RESTART-60`) — exact "appending below the existing transcript" contract. ✓
- **Restart via clicking the status-bar indicator** (SGR mouse `\e[<0;24;50M`/`m` on the `⟳ Restart` text, row 50) → "Terminal 2 restarted", tab live again. ✓ (primary documented UX)
- **Running terminal never restarted:** palette "Restart Terminal Process" on a LIVE terminal → status **"Terminal process is still running"**, no restart. ✓
- **Editor-restart survival of a DEAD terminal (`fresh` no-args pure restore):** workspace restored ("Restored 1 buffer(s) from previous session"), the exited terminal pane came back with its **transcript + a working `⟳ Restart terminal (exit 7)` indicator** (the exit code survived the editor restart), **dead-on-purpose** (not auto-restarted). ✓
- **Live-at-quit terminal:** restored LIVE (process present after restore, name `bash — ...` intact, scrollback preserved) — the loss below is specific to the exited-then-restored case.

## BUG #3074 (med, filed, tui-agent-auto-bug) — terminal tab NAME not preserved across editor restart
A terminal that **exited before quit** comes back after restart with a **generic `*Terminal N*` name** instead of its `bash — root@vm: /tmp/wr60` auto-title. Both `docs/features/terminal.md` §Terminal Persistence ("tab names are preserved across editor restarts") AND the 0.4.6 CHANGELOG ("terminal tab names now survive an editor restart", example `claude (exited)`) promise otherwise. `(exited)` marker + `⟳ Restart` indicator + scrollback all restore correctly — only the name is lost. **Impact (2-terminal repro):** `bash — .../subdir (exited)` + `bash — ... (exited)` → after restore BOTH become the SAME base name `*Terminal 0* (exited) 1` / `*Terminal 0* (exited) 2` — cwd distinction unrecoverable from the tab bar (only scrollback content tells them apart). Cause narrowed black-box: name is re-derived from the live process each time (restarting the terminal regains `bash — ...`; a dead restored terminal has no live process → shows internal buffer name). Clean 1-term + 2-term repros. Searches (0 dupes): `terminal tab name restart restore` / `terminal tab generic name exited restore session` / `terminal name survive editor restart`. Related-but-distinct: #2828 (scrollback bleed/frozen restore — closed/fixed), #2485 (read-only mode — closed), #2282 (rename-tab feature request).

## Preflight recheck — #2735 (our Run #59c bug, fixed 0.4.9) CONFIRMED FIXED (both halves), already maintainer-closed → recorded, not re-commented
- **Extract-Tab co-tenant survives restart:** `fresh main.rs lib.rs` (persistent) → palette "Extract Tab to New Workspace" on lib.rs → 2 co-tenant windows `wr60` + `wr60 (2)` (Orchestrator dock). Quit → **TWO** `workspaces/tmp_wr60.ws-<stable_id>-N.json` on disk (was ONE at file time). Relaunch `fresh` → **both windows restore** (dock shows both; Up-switch confirms `wr60`=main.rs, `wr60 (2)`=lib.rs). ✓
- **`--no-restore` writes no session state:** `fresh --no-restore main.rs lib.rs` → quit → NO `.ws-*.json` layout, only `workspaces/tmp_wr60/trust.json` (separate trust-persistence feature); relaunch `fresh` → empty `[No Name]` (nothing leaked). ✓

**HARNESS notes (Run #60):** (1) palette `C-u` clears the leading `>` too, dropping into FILE mode — reopen with Escape+`C-p` instead of `C-u` when you need command mode. (2) Status-bar element click is drivable: `⟳ Restart` text sits after " Trusted  Local  " (col ~18-24), row = pane height (50); SGR press `\e[<0;COL;50M` + release `...m`. (3) `pkill -f "fresh-master/target/release/fresh"` self-matches the running shell (exit 144) — use `ps aux | grep "[f]resh..."` to check instead, or Ctrl+Q the tmux session. (4) A live-at-quit terminal restores LIVE (process kept); only an exited-at-quit terminal restores dead — relevant when reasoning about "running processes terminated". (5) Ctrl+Q quits cleanly even with a live/dead terminal (no prompt). (6) Extract-Tab moves focus INTO the new co-tenant window (shows the extracted tab), source window holds the remainder.

**State updates:** run_log (this), learning_db (+1 topic: Restart-Exited-Terminal), github_issues (+#3074 row, #2735→FIXED row, header count 13→4-open +1 new #3074=5), confirmed_bugs (+BUG-039 #3074, BUG-038 #2735→FIXED), test_plan (Run #60 DONE + RUN #61 order; A9.1 + terminal-restart-survival + #2735 marked done).

**Cleanup:** killed tmux `fresh-run60`; removed `/tmp/wr60` + fresh workspace/terminal state; kept `/tmp/fresh-master` worktree + target for next run (container permitting).

---

# Run #61 — 2026-09-02 (v0.4.10 @ 7bcc8ff6c) — master FORCE-UPDATED (+~50 retained-mode render/input commits); Run Agent/New Workspace unified dialog COMPREHENSIVE PASS; BUG #3136 (terminal input replay) + BUG #3137 (Move-to-Folder popup keyboard regression); 0.4.10 key-routing fix VERIFIED

**Context/harness:** Rebuilt `/tmp/fresh-master` worktree; master still v0.4.10 but **force-updated** (Run #60's `992e61d02` no longer in history) with ~50 new commits = the **retained-mode UI migration** (panes/docks/menus/keyboard-layer rework — "Modal keys become containment", "Let a layer own the keyboard", "eb3061eaf Route the prompt, dock and panel keyboards through the widget tree"). High regression risk in exactly the emphasis areas → this run doubled as regression sweep. Emphasis per user directive: workspace orchestration + embedded terminals; vi ignored.

**Preflight:** state synced (Run #60 HEAD `9683362ef`); playbook + lessons intact (learning_db prose-headed since ~R#35 — consistent); GitHub auth LIVE (`issue_read` #3074 OK). **All 5 open agent issues (#3074 #2598 #2318 #2197 #2122) untouched by maintainer; no `(#NNNN)` fix commits in the new range → NO fixed-bug rechecks due.** Fixture `/tmp/wr61` (small git repo, main.rs/notes.txt/sub/inner.txt).

## (a) Run Agent… / Orchestrator: New Workspace unified dialog (0.4.5/0.4.6) — COMPREHENSIVE PASS (core) + 2 bugs in adjacent surfaces
- **Unified dialog confirmed from BOTH entry points:** palette "Run Agent…" opens `ORCHESTRATOR :: Run Agent`; flipping `Launch in: [Current workspace ▼]`→`[New workspace ▼]` morphs it into `ORCHESTRATOR :: New Workspace` (title + fields change live); palette "Orchestrator: New Workspace" opens the same dialog pre-set to New workspace. Footer: `Tab next/accept  S-Tab prev  ←→ change option  ↑↓ suggest/history  Space toggle  Enter advance/act  ^Enter create  Esc close`.
- **Agent dropdown cycle:** terminal → claude → codex → opencode → aider → custom…. Per-agent option rows: claude/codex/aider get `[ ] Auto mode (fewer approval prompts)`; opencode does NOT (docs: "reduced-approval mode where it has one"); all agents get Start prompt EXCEPT terminal/custom.
- **"Teach agent the Fresh CLI" checkbox** ([v] default) appears in the New-workspace Advanced section for claude/codex/opencode; **REMOVED for aider** — matches scripting.md ("aider cannot drive the editor at all … the option is not offered"). NOT present anywhere in Current-workspace (Run Agent) mode — no Advanced section there (fields: Launch in → Agent → Agent Command → [Auto mode] → [Start prompt] → Run → Cancel); doc ambiguity noted → potential_improvements.
- **custom… agent:** selecting it drops focus into the Agent Command field (the 0.4.6 "custom… having nowhere to type a command" fix works). Custom cmd `bash -c "echo CUSTOM-RA-MARKER; exec bash"` in CURRENT workspace → new tab, marker printed, live shell at workspace root. Plain `terminal` in current workspace → new tab `bash — root@vm: /tmp/wr61`, interactive (echo proof).
- **New-workspace mode fields:** Run in [Local][SSH][Kubernetes][Devcontainer] (←→ switch), Project Path (default = current root), Workspace Name (auto `wr61-N`, increments), Agent, `[ ▼/▶ Advanced… ]` (Enter toggles; state remembered) containing Agent Command / Teach / `[v] Create a new git worktree` / Checkout branch (`HEAD (no origin configured)`) / New branch name. Buttons `[ Create Workspace ] [ Create in Background ] [ Cancel ]`.
- **Create in Background — non-blocking VERIFIED (2×):** editor stays in the current workspace (tabs + terminal untouched); dock gains the new row; real git worktree created at `~/.local/share/fresh/orchestrator/tmp_wr61/<name>` on auto-branch `<name>`; visiting later shows a fully-set-up workspace with its terminal agent ALREADY running in the worktree root. **Create Workspace (visit)** switches immediately to the new workspace, terminal live at worktree root. Dock rows auto-name from their terminal (`wr61-1 · bash — root@vm: ~/...`).
- **Dialog state is STICKY/shared:** last Launch-in mode, Agent choice, custom command, Advanced expansion all persist across dialog opens AND across the two palette entry points. **NEAR-FALSE-POSITIVE (recorded so no future run re-derives it):** three new workspaces "mysteriously" showed `CUSTOM-RA-MARKER` in their fresh terminals → looked EXACTLY like cross-workspace scrollback contamination (#2828-family); actually the remembered `custom…` agent command legitimately re-ran in each new workspace. The Agent row DOES show `custom…` at creation time — visible, so sticky-default-by-design; surprise factor → potential_improvements (IMP-035). ALWAYS check the Agent dropdown before attributing foreign text in a new workspace terminal.

## BUG #3136 (med-high, filed, tui-agent-auto-bug) — terminal input replay via Run Agent dialog + workspace switch
Recipe (3/3): run a command in a workspace terminal (`echo BAIT2`) → `Ctrl+P` → **Run Agent…** → Enter (dialog opens) → Escape (cancel) → dock: switch to another workspace and BACK → the command line **re-appears at the shell prompt as PENDING PTY input** (no output). **Proven real input, not a render artifact: Enter re-executes it** (BAIT2 printed again); `C-u` clears it like typed text. Controls: bounce without the dialog → NO injection (2/2); palette query + Esc without opening the dialog → NO injection; second bounce without a new dialog-open → NO second injection (one-shot per dialog interaction). Same family (mentioned in body): a palette query string (`Spawn a new editor`) once appeared APPENDED to the dialog's Agent Command field on open (1×, not reproducible standalone). 0.4.9 changelog explicitly asks for input-anomaly reports after the fresh-input-parser rework.

## (b) 0.4.10 terminal key-routing fix — COMPREHENSIVE PASS
- **Focused live terminal keeps Ctrl+B/Ctrl+E for the shell:** with `cat -v` running, C-b/C-e echo `^B^E` (PTY delivery proven); File Explorer neither toggles nor steals focus. CONTROL: Ctrl+B from an editor buffer (main.rs) opens the File Explorer sidebar (menu bar gains "Explorer"; header later shows "(Ctrl+E)" hint).
- **File Explorer keys do NOT leak into a terminal PTY behind it:** armed the visible-behind terminal with pending `echo LEAKED` (no Enter); focused explorer (click `> sub` row); `Down`+`Enter` opened `sub/inner.txt` correctly; back in the terminal the pending line is INTACT and UNEXECUTED (no LEAKED output). PASS.
- **View → Terminal → Restart Terminal Process menu path (Run #60's untested third way):** exited terminal (`exit`) → tab `(exited)` + `⟳ Restart terminal` status element → F10 → View → Terminal > submenu (`Open Terminal / Close Terminal / Restart Terminal Process / Send Selection to Terminal / Toggle Keyboard Capture`) → Restart Terminal Process → same buffer, tab drops `(exited)`, live prompt, **old transcript preserved in scrollback** (C-Space+C-Home shows pre-exit lines; Ln 277). Status message says "Terminal 2 restarted" (internal id, not the per-workspace tab number — cosmetic, unfiled).

## (c)+(d) Terminal-tab extraction + orchestrator folders
- **B8.2 Extract a TERMINAL tab (`Extract Tab to New Workspace`):** works on a terminal tab → co-tenant `wr61 (2)` over the SAME project root (matches the command's description; test_plan's old "rooted at shell cwd" hypothesis DISPROVEN — no such behavior, not a bug). The terminal process survives extraction LIVE: same PTY (echo proof), **cwd preserved** (`/tmp/wr61/sub` after a pre-extraction `cd`), transcript intact, tab auto-title preserved (`bash — root@vm: /tmp/wr61/sub`). Status: `Extracted bash — … into workspace…`. **B8.5 trust inheritance:** extracted workspace status bar shows `Trusted` (source Trusted). PASS.
- **B7.6 folder search field:** dock `[Search Tasks]` filter with folders — typing `wr61-3` filters the tree to the match WITH its folder ancestry intact (`▼ ▤ FolderA → ▼ ▤ SubB → · wr61-3`); other workspaces hidden; C-u clears. PASS.
- **New Subfolder nesting:** F2 on a folder header (dock focused via M-o) → `Rename… / New Subfolder… / Delete Folder` menu — ↑↓/Enter WORK in this menu → New Folder dialog (name field + `[v] Organize "<ws>" under this folder` + Cancel/Create Folder) → nested `FolderA ▸ SubB ▸ wr61-3` rendered + workspace organized in. PASS (via mouse for the button; typing works in the dialog).
- **Drag-move workspace between folders:** SGR press/motion/release drag on dock rows = no effect (no documented contract; `[~]` harness-limited, unfiled).

## BUG #3137 (med, filed, tui-agent-auto-bug) — Move-to-Folder popup keyboard regression
The **Move to Folder…** chooser popup (`● Top level`/`New Folder…`, footer `↑↓ choose Enter select Esc cancel`) does NOT own the keyboard: `↓` falls through to the dock list behind it — dock selection moves = **live workspace switch** — and the popup dismisses (3/3; ANSI-verified the `48;5;25` highlight lands on the dock row, popup rows never highlight). Keyboard-only folder filing impossible; mouse click on the popup rows works (workaround). REGRESSION: #2694 (closed, PR #2696) explicitly listed this dropdown among those supporting ↑↓ — almost certainly the retained-mode keyboard-layer rework.

**Unreproduced anomalies (PENDING, do not file):** (1) Enter on collapsed `[ ▶ Advanced… ]` once closed the whole dialog while toggling the remembered state (1×; 2 deliberate retries toggle correctly in place). (2) First-char-after-focus-change swallow: 4 organic occurrences (`at -v`, `cho LEAKED`, `cho EXTRACTED`, `cho R3-MARK` — first char eaten after click-into-terminal or workspace switch) but **0/9 in controlled trials** (grid-click / tab-click / dock-switch × 0.6s and 1.5s gaps) → trigger involves richer preceding UI state (palette/dialog interplay — plausibly the same input-routing family as #3136). HARNESS RULE: after any click/workspace switch, send a sacrificial char or verify the typed line before Enter.

**HARNESS notes (Run #61):** (1) palette ranks recently-run commands higher — `Down`+`Enter` after typing a query can select a DIFFERENT item than last time (accidentally ran Extract-Tab instead of New-Workspace); always capture the highlighted row before Enter, or type an unambiguous query (e.g. the command's description text: `Spawn a new editor`). (2) With the File Explorer open the tab bar shifts right (~col 95+) — recompute tab click columns. (3) Dock row click = SGR `\e[<0;5;ROWM`+`m`; dock focus for F2 needs `M-o` (a header click alone doesn't give the dock the keyboard). (4) `[ ▼/▶ Advanced… ]` Enter toggles in place; dialog buttons: Tab to `▸ [ Create Workspace ]` etc. + Enter. (5) In-dialog text fields accept `C-u` clear + literal typing; `C-a`/`C-k` then immediate `-l` text can eat the first char — pause between.

**State updates:** run_log (this), learning_db (+1 topic: Run Agent dialog + input-replay + folders), github_issues (+#3136 +#3137 rows, header), confirmed_bugs (2 filed + pending observations), potential_improvements (+IMP-035 sticky custom agent, +IMP-036 Teach-toggle absent in current-workspace mode), test_plan (Run #61 DONE + #62 NEXT + checkbox updates).

**Cleanup:** quit editor (C-q), killed tmux `fresh-r61`, removed `/tmp/wr61`, orchestrator worktrees + terminal state for tmp_wr61; kept `/tmp/fresh-master`.

**NEXT (Run #62, top-down, emphasis-first):** (a) **Retained-mode regression sweep of remaining keyboard-owned surfaces** (the #3137 family): New Task… ▾ dropdown, project dropdown Alt+P, Filters fold, Manage modal buttons, tab context menu + '+' popup grabs (0.4.2 claims re-verify on 0.4.10), Settings dialog keys — cheap captures, high regression odds. (b) **#3136 adjacent characterization if maintainer engages** (does the replay hit agent terminals / New Session dialog too?). (c) **A10.6/A10.7 mouse config toggles** (`terminal.mouse_drag_selects=false` inert; `terminal.mouse_forwarding="alt_screen"`), **A8 Ctrl+Click OSC7 re-verify on 0.4.10**, A11.3 #2609/#2608. (d) **B9 crash resilience (#2658)** kill -9 → orchestrator remembers workspaces; trust prompt wording. (e) A5.2/A5.3 F9 capture + Ctrl+P routing; A3.3 scrollback keys explicit. (f) `fresh --skill` (0.4.10 new) smoke + `fresh --cmd script run` basics (scripting.md examples) if terminals allow. Watch for maintainer action on #3136/#3137/#3074/#2598/#2318/#2197/#2122.
