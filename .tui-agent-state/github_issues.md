...
Last updated: Run #46, 2026-07-06 (v0.4.3 @ 9f6135001. Filed **#2577**: Virtual space — status-bar Ln/Col indicator freezes at the line-end position while the cursor is in virtual space. Rechecked 5 closed agent issues: **#2309/#2314/#2315/#2420 CONFIRMED FIXED** via UI; **#2312 STILL REPRODUCES** in v0.4.3 (occurrence highlight still fixed color 16) → commented on the closed issue, did NOT re-file.)
Prev: Run #45 (Filed #2443: vi mode dot-repeat `.` of `o`/`O`/`a`/`A` corrupts the buffer.)

---

## Open Issues (agent-filed)

| [#2577](https://github.com/sinelaw/fresh/issues/2577) | Virtual space: status-bar Ln/Col indicator freezes at the line-end position while the cursor is in virtual space | Run #46 | **Open** | Found testing the new Virtual space feature (0.4.3 headline, master @ 9f6135001). With `editor.virtual_space` on, moving the cursor past EOL (horizontal) freezes the `Col` readout at the EOL column while the real cursor advances (proven via `tmux #{cursor_x}` + typing a marker that lands at the true column); vertical virtual space (ArrowDown below last line) freezes BOTH `Ln` and `Col`. Readout only corrects when the cursor returns to real text. Reference = Visual Studio (feature's stated model) reports the virtual column. Distinct trigger/symptom from #2301. Low–med usability; the position indicator is wrong for the whole duration in the mode built for free positioning. Workaround: type to materialize, or move back to real text. Do NOT re-file. |
