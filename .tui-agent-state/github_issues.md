...
Last updated: Run #44, 2026-06-22 (Filed #2441: vi mode find-char `df`/`dt`/`cf`/`ct` hang in operator-pending or mis-delete + `;`/`,` repeat no-op; #2442: vi mode `j`/`k` onto a shorter line park cursor past EOL → `x` joins lines — both found continuing the vi-motion sweep on master @ 3b8c2eca1, v0.4.1.)
