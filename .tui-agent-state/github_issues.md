...
Last updated: Run #43, 2026-06-22 (Filed #2438: vi mode indent operators `>>`/`<<`/visual `>`/`<` are no-ops; #2439: vi mode quote text-objects `i"`/`a"` don't search forward on the line — both Vim-compat bugs found continuing the vi-motion sweep on master @ 8ee2baf31, v0.4.1.)
