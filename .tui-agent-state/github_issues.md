...
Last updated: Run #45, 2026-06-22 (Filed #2443: vi mode dot-repeat `.` of `o`/`O`/`a`/`A` corrupts the buffer — injects unrelated line content instead of replaying typed text, while `i`/`x` dot-repeat are correct; found finishing the vi-command sweep on master @ 1c6bd8ce9, v0.4.1. Same sweep confirmed `o`/`O`/`s`/`S`/`D`/`C`/`Y`/`*`/`#`/`n`/`N`/count+`G` all WORK — gap list now only `R` + `gU`/`gu`/`g~`.)
Prev: Run #44 (Filed #2441 vi find-char + operators; #2442 vi `j`/`k` past-EOL → `x` joins lines.)
