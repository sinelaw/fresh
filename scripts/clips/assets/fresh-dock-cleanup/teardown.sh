...
# daemon dies, but the *registry entry* under XDG_DATA_HOME/fresh/workspaces
...
SESSION="${CLIP_SESSION:-fresh-dock-clip}"
...
# 2. any process the clip left running in a workspace terminal
for p in $(ps -eo pid,args | awk '/clip-agent|fresh-dock-clip\/bin\/busy/ && !/awk/ {print $1}'); do
  kill "$p" 2>/dev/null || true
done

# 3. unregister the worktrees before the tree goes
if [ -d "$CLIP_HOME/tree/projects" ]; then
  for r in "$CLIP_HOME"/tree/projects/*/; do
    [ -d "$r/.git" ] || continue
    git -C "$r" worktree prune >/dev/null 2>&1 || true
  done
fi

# 4. the staged tree, the per-clip XDG root (registry included), the rendered
#    captures. The XDG root is what holds workspaces/, so removing it is the
#    "drop the on-disk registry entry" step — there is no daemon left to ask.
rm -rf "$CLIP_HOME"
