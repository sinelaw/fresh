...
Two caveats on where the write lands. The toggle always writes the *user* config layer — if a project config (`.fresh/config.json`) sets the same key, the project value still wins in that project, so the toggle will appear to revert there on restart while the new default applies everywhere else. And a few settings (line numbers, line wrap, inlay hints) are also snapshotted per-workspace, so an existing workspace can restore its own last-used value over a default you changed elsewhere.
