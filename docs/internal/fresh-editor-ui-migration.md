...
| B.4 | `Editor::shell_pointer_event` — the event travels on the editor, not in the message. **Three readers left of four.** | Delete it. It exists because the interiors tell a drag from a move; the library routes drags by **pointer capture**, so they stop asking. **One reader was never that**: `press_pane_content` reached back for the press's *modifiers*, which the tree reports on `Event::mods` beside the `Event::clicks` the same fact already carried — so `PaneContentPress` carries them and `crossterm_mods` converts once, where `mods` already converts the other way. The three that remain are the entry's real subject: the live terminal's own mouse protocol, which wants the crossterm event rather than a fact about it, and the two modal interiors still hit-testing their painters' rectangles. **A fourth is blocked on the library**: the settings map row activates on a *double* click and reads the doubleness from here, but it arrives through a `List`'s row activation and `on_activate_handler` is handed an index and no event — carrying the click count there is a change to the widget's activation contract, not a field on a fact. | Generalising it into a typed side channel. "Routed, not transported" was a transitional apology, not a design. |
