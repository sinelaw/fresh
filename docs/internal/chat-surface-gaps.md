...
> _**PLANNED — none of this is implemented**, and the chat surface it
> describes is not on `master` either: it was built on a branch that was
> closed unmerged (see [agent-control-plane.md](agent-control-plane.md) for
> why). Read "the chat does X today" as *did X on that branch*._
>
> _**Several items below have since been answered by the framework.** The
> widget rewrite on `master` supplies three things this punch list asks for by
> hand: `Popup { anchor, screen_space }`, whose own API docs say the host
> "resolves the final screen rect — opening below the anchor, flipping above
> near the frame edge, clamped on screen"; `full_width`, which stretches a
> button to its container instead of padding its label out to a width the
> caller had to measure; and scroll payload carried on the layout box. Between
> them those cover the popup placement and chrome items, the dock-width
> mismatch behind the wrapped-toolbar bug, and the transcript's "recomputed
> slice rather than a scrollable region" root cause. Re-read each item against
> the current widget vocabulary before implementing it — the defects were
> real, but several of the diagnoses name machinery that no longer exists._
>
> _A punch list of accepted defects and agreed changes to the agent chat,
> recorded so the reasoning survives until someone picks them up._
