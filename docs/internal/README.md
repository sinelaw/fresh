...
| [finder-preview-highlighting-design.md](finder-preview-highlighting-design.md) | Putting syntax colours on the composed preview and result buffers the grep-style finders show (sinelaw/fresh#3196), the follow-up to the diff highlighting of sinelaw/fresh#3104. Audits which of the three result surfaces is already coloured and why the other two cannot be, then designs both on the `setSyntaxRegions` mechanism: one region per gutter run with a *measured* prefix for the contiguous preview pane, one region per row for the Quickfix list, and a fresh parser stream per refresh so a window that starts mid-file inherits nothing. Carries the test properties and the stated limits. Entirely planned. |
