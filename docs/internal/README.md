...
| [plugin-window-scope-plan.md](plugin-window-scope-plan.md) | Answer to sinelaw/fresh#3326: the sidebar sections that showed one window's Markdown outline in another (and, on a click, forked the file into a second independent buffer), the window switch that fires no `buffer_activated`, the "Panel unavailable" placeholders that never expire, and the focus cycle no keymap binds. Diagnoses the plugin seat — ids and hooks with no window attached, silent active-window resolution, lifecycle left to each plugin, ownerless panels — and lays out a phased plan: one focus announcer, window ids in every identity plugins hold, scope-owned chrome with the narrow scope as default, chrome-focus as a signal, keyboard reach, and the invariants the host checks. |
