...
| [widget-controls-own-interaction.md](widget-controls-own-interaction.md) | The rules that make the shared widget controls own their interaction: the focused control answers a key before the plugin's mode (with declared dialog-wide shortcuts as the exception), the host owns focus and pop-up state, one shared pop-up, the text area's caret visibility, spatial arrow navigation, and sizes from layout — with the plugin workarounds each rule deleted. |
