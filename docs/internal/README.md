...
| [chat-surface-gaps.md](chat-surface-gaps.md) | Punch list for the agent chat (the Home column and the dock section over one transcript): dropping the completion popup's vertical borders, dropping the composer's `to` label and `[ ]` wrappers, re-laying the transcript so the author sits on its own row and the body wraps flush to the full chat width, and the narrow-dock layout bug where a wrapped toolbar pushes the composer off the bottom edge. Records which items are host-side widget changes versus plugin-side, and the shared root cause — the plugin sizes against the dock's *default* width, never the rendered one. Nothing here is implemented. |
