...
| [shell-stylesheet-design.md](shell-stylesheet-design.md) | How a second backend could draw a *control* rather than re-colour a string. Today a plugin's button reaches the screen as text — the widget runtime formats `[ label ]`, frame and focus marker included — so the display list says how a run is painted and never what it is: the web cannot make a button a button, and a card's outline cannot be dimmed without dimming the names beside it. Proposes a **class** naming what a thing is, with each backend owning a table from class to appearance (Rust for the terminal, CSS for the web) and the description carrying only the naked label. Shows that most of the mechanism already ships — `resolve` is already a per-item, backend-owned stylesheet, a themed region already paints a fill over its own rectangle, the fold already draws borders, and `fresh_ui::widgets::Button` already speaks in classes — and that what is missing is a property set, not a home. Unifies affixes and boxes as one **cell box model** (`border` / `padding` / `ink`, with geometry derived from the rule rather than declared beside it), splits resolution into the builder's phase and the fold's, draws the line between structure (the description's) and appearance (the class's), and answers the documented objection that killed class names before — the blow-up was the absence of a cascade, not the presence of names. Five staged steps with the terminal's cells as the invariant, plus the decisions taken: the theme key stays and the class sits beside it in its own slot, the widget runtime's own button path migrates so the duplication has an end date, and the class vocabulary is **open** — unlike draw kinds, an unrecognised class matches no rule and paints as it did, which is how CSS degrades and what lets a class be invented for the web alone. Planned. |
