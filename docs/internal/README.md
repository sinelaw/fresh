...
| Doc | What it covers |
|-----|----------------|
| [key-vocabulary-unification.md](key-vocabulary-unification.md) | Collapsing the several independent parsers, name tables and spellings that turn a *string* into a key — the config's key/modifier fields and chord arrays, a plugin mode's `C-x C-s` binding strings, the widget wire's `S-Left`, and each widget kind's own literal match arms — into one parser over one table with one compact syntax, in the data layer where the most constrained consumer lives. Carries the survey that motivates it (five parsers, three tables, and three spellings each for Escape and Shift+Tab), the rule that makes it safe (**parsing widens, printing narrows**: accept the union of every spelling that ever worked, emit exactly one), the surface-by-surface compatibility argument for user configs, built-in keymaps, the two public plugin APIs, the write-back path and the generated schema and reference, and an explicit list of what stays *outside* the chokepoint (the terminal decoder, the calibration file's on-disk form, free-form hint-bar labels). Names two live "registers fine, never fires" bugs it settles — a panel plugin's `S-Tab` that no widget has ever matched, and the review mode's `z a` chord that dies whenever a sidebar holds the keyboard because a panel's keymap resolves single keys only — and separates the one that is a parsing fix from the one that needs chord state added to the panel keymap. Six steps; the middle four widen what parses and change nothing else. Planned. |
...
| [plugin-window-scope-plan.md](plugin-window-scope-plan.md) | The model behind sinelaw/fresh#3326: one focus announcer, the window on every buffer id a plugin holds, sidebar sections scoped to a window or a buffer with the narrow scope as default, and chrome focus as a signal. |
| [orchestrator-layout-asks.md](orchestrator-layout-asks.md) | Open-work register for the Orchestrator dock and dialogs after they landed on the retained-mode tree: where the plugin measured a width the host already had, and what went wrong when it did — a footer and a popup that kept a stale fit across a resize, a table that padded without truncating, a form built to be counted. Draws the line between the widths the wire format is *designed* to carry and the second layout `retained-mode-ui.md` exists to remove. Most of it is now closed, each item recording what landed and why; the one library ask that survived review is main-axis justification on a wrapping box, and the register says plainly why the container that would reserve its tallest alternative is **not** worth building. What remains open is named at the end, and waits on master's keyed geometry index. |
| [agent-control-plane.md](agent-control-plane.md) | Making one agent drive the whole environment: removing ambient active-window scoping from the plugin dispatch layer in favour of explicit ids, a whole-environment read (`describeEnvironment`, terminal screen/tail), a "needs you" activity state, the Home view — a chat carrying what every agent has said beside the agent list and the selected agent's live terminal — built on the shipped cross-window live-render primitive, and a per-agent mailbox for messaging peer agents of other vendors. Adds no new CLI verbs — everything lands on the existing script channel. Successor to the CLI-exposure plan below. **Planned in full — built on a branch that was closed unmerged, and the widget-v2 rewrite supersedes its host-side plumbing; the design still stands.** |
| [agent-delivery-hooks.md](agent-delivery-hooks.md) | What it takes to hand a message to an agent that is *already running* — waking it if idle, queueing if busy, and never answering a pending permission prompt — across Claude Code, Codex and OpenCode. OpenCode's `prompt_async` is verified end to end including the permission case; Claude Code's cross-session messaging socket is purpose-built but its message frame is undocumented; Codex cannot be reached while idle at all. Records the launch-time change each vendor forces on Fresh (assigned port and session id; inline `--settings`; per-agent `CODEX_HOME` because a linked worktree's own hooks are silently discarded). Planned. |
| [chat-surface-gaps.md](chat-surface-gaps.md) | Punch list for the agent chat (the Home column and the dock section over one transcript): dropping the completion popup's vertical borders, dropping the composer's `to` label and `[ ]` wrappers, re-laying the transcript so the author sits on its own row and the body wraps flush to the full chat width, the narrow-dock layout bug where a wrapped toolbar pushes the composer off the bottom edge, the mouse wheel scrolling the workspace tree wherever the pointer is, and the missing transcript scrollbar. Records which items are host-side widget changes versus plugin-side, and the two shared root causes — the plugin sizes against the dock's *default* width rather than the rendered one, and the transcript is a recomputed slice rather than a scrollable region. Nothing here is implemented. Planned; several items are now answered by widget v2 (`Popup{anchor}`, `full_width`, scroll on the box). |
