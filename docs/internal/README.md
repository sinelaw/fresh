...
| [agent-control-plane.md](agent-control-plane.md) | Making one agent drive the whole environment: removing ambient active-window scoping from the plugin dispatch layer in favour of explicit ids, a whole-environment read (`describeEnvironment`, terminal screen/tail), a "needs you" activity state, the Home/Fleet view built on the shipped cross-window live-render primitive, and a capability-routed `delegate` for messaging peer agents of other vendors. Adds no new CLI verbs — everything lands on the existing script channel. Planned; successor to the CLI-exposure plan below. |
