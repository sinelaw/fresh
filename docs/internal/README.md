...
| [agent-delivery-hooks.md](agent-delivery-hooks.md) | Whether the mailbox's delivery half can move onto the coding agents' own lifecycle hooks, so a message reaches an agent without anyone typing into its terminal. Surveys what Claude Code, Codex and OpenCode actually expose (Claude Code's and Codex's `Stop` can hand over text and refuse to let the agent go idle; OpenCode's hooks cannot inject, but its HTTP server can). Proposes keeping the mailbox as the store and making hooks the trigger, which disposes of the double-delivery and permission-prompt hazards that left the terminal-injection route half-built. Planned. |
