...
`BufferFormat` stores **both** current and original encoding/line-ending; change-detection compares them, and a promote step rebaselines after a successful save. The save recipe builder computes whether line-ending and encoding conversion are needed (the latter true whenever the encoding is not plain UTF-8/ASCII, since storage is always UTF-8), prepends the original BOM as the first insert, and for each piece either emits a zero-copy copy for unchanged `Stored` regions from the same file or re-encodes/re-line-ends the bytes into an insert. Binary files skip conversion, preserving raw bytes.

Writing the recipe is the save path's job, not the filesystem's (`model/buffer/save.rs`). A remote file gets the recipe itself (`write_patched`) so the agent rebuilds it server-side. A local file goes through `save_local`, the one place that chooses how: by default an **atomic replace** (`replace_file_preserving_identity`: temp file + rename, the temp file given the original's permissions, owner, group and xattrs first, content-bound xattrs such as file capabilities excepted), so a crash leaves the old content or the new. When the new file couldn't stand in for the old one, the file is instead **overwritten in place**, keeping its inode: when it is owned by another user (checked up front — only root could give a new file their ownership), or when the replace reports an `IdentityLoss` (other hard links; an owner, group or xattr it can't set) and refuses without touching the file.

An in-place write first **stages** a complete copy of the new content, private to the user, in the recovery directory (or, when that can't be written — unwritable or read-only, or its disk full; any other error there fails the save — next to the file, else in the system temp dir), with `<path hash>.inplace.json` metadata pointing at it; the copy goes once the write completes, and stays if it fails part-way. There is one staged copy per file: a new one supersedes the last only once the file is open for writing, since until then the earlier copy may be all that is left of a torn file. If nothing can be staged anywhere, a fully loaded buffer is still written in place, unless the disk is full; a large file's save, whose recipe reads the unchanged parts back from the file it overwrites, is refused with an error saying the copy couldn't be staged. A large file's save reads the parts it never loaded back from the file they were loaded from (Copy ops, or the reads of a conversion), whatever file it writes to; so it is refused, Save As included, while that file may be torn and reading it at the offsets it had when loaded would write the wrong bytes: while a staged copy for that file exists and the file doesn't match it, or while this buffer's own in-place write of it failed part-way (`TornWrite`). The buffer remembers the latter (`Persistence::source_torn`, cleared by a successful save), since the metadata that would say so may not have been writable. When the file or its directory can't be written at all, the content goes to a private temp file owned by the `SudoSaveRequired` error, and the sudo prompt writes it with the original owner/permissions (resolving any staged copy of that file). The recovery directory is the editor's (`DirectoryContext::recovery_dir`), passed to `TextBuffer::save`; at startup the editor sweeps it (through the local filesystem) for what crashed saves left and no longer need — a staged copy its file already matches, metadata whose copy is gone, dead processes' temp files — and then offers any staged copy that differs from its file to the user in an **Interrupted Save** dialog (`Editor::offer_interrupted_save`: restore the file from it, show the diff, discard it, or decide later), removing the copy and its metadata once they decide. The "Review Interrupted Saves" command asks again, including about a copy this session's own save kept (`InplaceWriteRecovery::is_settled`: ours, or its process gone — saves are synchronous, so none of ours is in flight). Restoring rewrites the file behind the back of buffers that have it open, so every unmodified buffer of it, in every window, is reloaded; a modified one keeps its changes and, its recorded mtime no longer matching, counts as changed on disk. A staged copy is recognised by its name (`.inplace-<name stem>-<pid>-<timestamp>.tmp`), not by where it is, since the temp dir can move between sessions; which copies hold up saves, are offered, and survive the sweep is one test (`is_kept_copy`: staged for its file, still there, and different from it), so a copy that blocks saves is always one the user is asked about. The temp names and the in-place metadata record the host (`host_id`: host name plus pid namespace) as well as the pid, since the data directory may be shared by several hosts or containers: another host's entries count as live until they are a week old.
