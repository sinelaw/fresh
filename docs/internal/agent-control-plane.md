...
> **Status: PLANNED in full — none of this is on `master`.**
>
> An implementation was carried a long way on a branch: the explicit-id
> terminal APIs and `describeEnvironment` (§4, §6.1, §6.2), the
> discoverability fixes (§9), the `waiting` state (§7.1), the agent list
> (§7.2), the `pane` widget (§6.5, §6.6), and §10's phases A, B and C — the
> event stream and `script run --as`, the mailbox, and Home. That branch was
> **closed unmerged**, so where the text below says something "landed" or
> "ships", read it as *was built and demonstrated, then withdrawn*.
>
> It was withdrawn because the widget framework was rewritten underneath it:
> the box-tree work that split widget hit identity from geometry, moved the
> plugin panels onto `fresh-ui` capabilities, and split the data layer into
> `fresh-editor-core`. That rewrite supplies as framework what the branch had
> built by hand — most visibly `Popup { anchor }`, which resolves its own
> screen rect and flips above the anchor near the frame edge, where the branch
> carried a host-side "draw the completion popup upwards" flag.
>
> **The design in this document is not what was invalidated; the plumbing
> under it was.** A second attempt should re-derive §6 and §7 from the new
> widget vocabulary rather than port the old host changes.
>
> Kept because the reasoning, the constraints and the phasing are the
> expensive part and they still hold.
