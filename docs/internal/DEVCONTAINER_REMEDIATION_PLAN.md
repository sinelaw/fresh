...
| F3 | Medium | The clickable Remote Indicator was opt-in: no palette command, no default keybinding | **Partially fixed** — added `Show Remote Indicator Menu` palette command, F6 default keybinding, and locale strings. **Did NOT** add `{remote}` to `default_status_bar_left` because it broke 14 unrelated 80-col-pinned status-bar tests (the leading ` Local ` element pushes diagnostics/messages off the bar at 80 cols). Users who want the visible widget still get it via the v1→v2 migration on upgrade, or by adding `"{remote}"` to their config manually. |
