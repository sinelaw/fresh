...
> corrections are listed at the end. The mailbox this assumes as the store is
> described in [agent-control-plane.md](agent-control-plane.md) §8.1 — it was
> built on a branch that was closed unmerged, so it is not on `master` either.
> Nothing in the vendor findings below depends on that: they are properties of
> Claude Code, Codex and OpenCode, not of Fresh._
...
The mailbox design satisfies (b) and (c) and fails (a) — a file lands in `inbox/`,
