...
fn test_debug_truncate() {
    ...
    println!("History: len={}, current_idx={:?}, can_back={}, can_fwd={}",
             hist.len(), hist.current_index(), hist.can_go_back(), hist.can_go_forward());
    ...
    println!("After first back: content = {:?}", harness.get_buffer_content());
    let hist = &harness.editor().position_history;
    println!("History: len={}, current_idx={:?}, can_back={}, can_fwd={}",
             hist.len(), hist.current_index(), hist.can_go_back(), hist.can_go_forward());
    ...
    harness.send_key(KeyCode::Left, KeyModifiers::ALT).unwrap();
    println!("After second back: content = {:?}", harness.get_buffer_content());
    let hist = &harness.editor().position_history;
    println!("History: len={}, current_idx={:?}, can_back={}, can_fwd={}",
             hist.len(), hist.current_index(), hist.can_go_back(), hist.can_go_forward());
    ...
    println!("After forward: content = {:?}", harness.get_buffer_content());
    let hist = &harness.editor().position_history;
    println!("History: len={}, current_idx={:?}, can_back={}, can_fwd={}",
             hist.len(), hist.current_index(), hist.can_go_back(), hist.can_go_forward());

    // Should still be in Buffer 4 (at the end of history)
    assert_eq!(harness.get_buffer_content(), "Buffer 4", "Should still be in Buffer 4 after forward");
}
