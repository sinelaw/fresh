# TODO: Plugin System Maturity

This document outlines the next steps needed to bring the plugin system to maturity, enabling complex plugins like undo history visualization, Magit, and other Emacs-style major modes.
...
**Example Use Case: Magit Log**
```lua
editor.spawn("git", {"log", "--oneline", "-n", "50"}, function(stdout, stderr, exit_code)
    if exit_code == 0 then
        -- Parse git log and display in custom buffer
...
**Example Use Case: Undo History Visualization**
```lua
...
**Example Use Case: Magit Rebase Mode**
```lua
...
### 1. **Undo Tree Visualizer**
- Uses: Buffer query, virtual buffers, custom keybindings, undo history access
- Shows branching undo history graphically
- Navigate and jump to any state

### 2. **Magit (Git Interface)**
- Uses: Async processes, popups, custom modes, virtual buffers
- Git status, log, diff, commit, rebase, etc.
- Full interactive git workflow

### 3. **Project Search & Replace**
- Uses: Async search, virtual buffers, multi-buffer operations
- Ripgrep integration
- Preview and apply changes

### 4. **LSP Code Actions Menu**
- Uses: LSP integration, popups, buffer modification
- Custom code action UI
- Refactoring tools

### 5. **Snippet System**
- Uses: Buffer modification, keybindings, state management
- Tabstops and placeholders
- Custom snippet definitions

### 6. **File Tree / Explorer**
- Uses: Virtual buffers, custom keybindings, async file operations
...
- [ ] Complete API reference (all functions, parameters, return values)
- [ ] Plugin development guide (step-by-step tutorials)
- [ ] Best practices document
- [ ] Migration guide (when breaking changes occur)
- [ ] Example plugin collection (commented, well-structured)

---

## Priority Ordering for Next Work Sessions

1. **Buffer Query API** - Foundation for most interactive plugins
2. **Async Process API** - Unblock git operations and external tools
3. **Popup/Selection API** - Enable interactive UIs
4. **Mode System** - Allow mode-specific behavior
5. **Enhanced Hooks** - Better plugin lifecycle
6. **State Management** - Persistent plugin configuration
7. **Error Handling** - Improve stability and debugging
8. **Performance** - Optimize for large buffers
9. **WASM Support** - Future extensibility
